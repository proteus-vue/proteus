...
const ROUTE_RULES: RouteRule[] = [
  ...
  {
    id: 'route/guard',
    title: '路由守卫（实例级三端一致 API）',
    description: 'router.beforeEach/afterEach 注册守卫（delegate 全局注册表）；返回 false 取消导航；--trace-router 输出守卫链路',
    why: '基于 meta.needLogin + 全局 guards 的跨端导航控制（路由规划 M6 §2）；三端 API 一致',
    when: '导航前/后（push 调用守卫链）',
    example: { before: 'push("order")（meta.needLogin）', after: '[guard] beforeEach → order 被拦截 / 放行' },
    verify: 'tests/router-guards-tabbar.test.ts 守卫用例',
    source: 'packages/router/src/guards.ts + index.ts（Router.beforeEach）',
    decision: '#101',
  },
  {
    id: 'route/tabbar',
    title: 'tabBar（config 驱动三端产物）',
    description: 'proteus.config router.tabBar → Web <TabBar> / MP app.json.tabBar（pagePath 匹配路由 name）/ App 原生栈；超 5 项告警降级',
    why: 'tabBar 唯一声明源（路由规划 M6 §3）；MP 原生限制（首页级 + 5 个）编译期告警',
    when: 'config 声明 router.tabBar 时',
    example: { before: 'tabBar: { list: [{ name: "home", text: "首页" }] }', after: 'app.json.tabBar.list[0].pagePath = "home"' },
    verify: 'tests/router-guards-tabbar.test.ts tabBar 用例',
    source: 'packages/router/src/codegen/mp.ts → generateMpConfig',
    decision: '#101',
  },
  {
    id: 'route/redirect',
    title: 'redirect 跨端处理',
    description: '<route>.redirect → Web vue-router redirect 原生 / MP 页配置标注（运行时 onLoad redirectTo 模拟）/ App replaceScreen',
    why: '小程序无原生 redirect，运行时模拟（路由规划 M6 §5）；与 parent 互斥（schema 已校验）',
    when: '<route> 声明 redirect 时',
    example: { before: 'redirect: "/new"', after: 'Web: redirect: "/new" / MP: 页配置 redirect 标注' },
    verify: 'tests/router-guards-tabbar.test.ts redirect 用例',
