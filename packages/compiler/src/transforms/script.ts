...
export const SCRIPT_RULES: TransformRule[] = [
  ...
  },
  {
    ...
    phase: 'script',
    status: 'implemented',
    title: 'defineProps → Component properties（v0.3 组件系统）',
    description: '组件模式：defineProps({ label: String, initial: { type: Number, default: 0 } }) → properties（type + 默认值 value）；props.xxx 访问重写为 this.data.xxx',
    why: '小程序组件用 Component({ properties }) 声明外部属性；Vue 组件 props 编译期映射（v0.3，决策 #79）；MVP：仅对象形式（TS 泛型形式警告）',
    when: '组件文件（isComponent）含 defineProps 时',
    example: { before: 'const props = defineProps({ initial: { type: Number, default: 0 } })', after: 'properties: {\n  initial: { type: Number, value: 0 },\n}' },
    verify: 'tests/mp-transform.test.ts 组件系统用例',
    source: 'packages/compiler/src/script.ts → extractProps',
