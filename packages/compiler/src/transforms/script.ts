...
export const SCRIPT_RULES: TransformRule[] = [
  ...
  {
    id: 'script/computed-observer',
    phase: 'script',
    status: 'implemented',
    title: 'computed 派生值的依赖观察者（observers 重算 setData）',
    titleEn: 'Observers for computed derived values (recompute setData via observers)',
    description: '组件内 computed 的派生值（如 isChecked/boxStyle/columns）除在 ready() 初始化一次外，还按**依赖字段**生成 Component observers —— 依赖（props 或 data 字段）变化时重算并 setData；同一字段的多个 computed 合并进一个 observer；依赖提取用「字段引用扫描」（覆盖 this.data.f / f.value / props.f 三种写法）',
    descriptionEn: 'Besides a one-time initialization in ready(), a component computed derived value (e.g. isChecked/boxStyle/columns) also emits Component observers keyed by its dependency field — when the dependency (a prop or a top-level data field) changes, it is recomputed and setData is called; multiple computeds on the same field merge into one observer; dependency extraction uses field-reference scanning (covering this.data.f / f.value / props.f)',
    ...
    whyEn: 'Mini programs have no reactive system: a computed used to be evaluated only once in ready(), so after a parent updates props or data changes the derived value was never recomputed, leaving template class bindings (which read the derived value) visually stale. Real-device evidence: p-checkbox fires change but the UI does not change (its class reads the derived isChecked); p-switch happened to work only because its class reads modelValue directly',
    when: '组件（isComponent）内存在 computed，且其依赖为 props 或顶层 data 字段时',
    example: {
      before: 'const isChecked = computed(() => props.modelValue)  // 仅在 ready() 初始化一次',
      after: 'observers: {\n  modelValue(n, o) {\n    this.setData({ isChecked: this.data.modelValue })\n  },\n}',
    },
    verify: 'tests/mp-transform.test.ts（computed observer 生成用例）+ showcase 真机（p-checkbox 动态 props → UI 更新）',
    source: 'packages/compiler/src/script.ts → observers 发射（computedDeps 字段引用扫描 + computedInitLine 复用）',
    decision: 'SOP v2 批次 1（p-checkbox UI 不更新真机 bug 修复，2026-09-13）',
  },
  {
    id: 'script/ref-nested-write',
    phase: 'script',
    status: 'implemented',
    title: 'ref 对象嵌套写补 setData（o.value.x / o.value[k] = v）',
    titleEn: 'Nested ref writes emit setData (o.value.x / o.value[k] = v)',
    description: '方法体中 `o.value.field = expr` / `o.value[key] = expr`（ref 持有对象/数组的字段写）→ 改写为 `this.data.o.field = expr` **并补 `this.setData({ o: this.data.o })`**；整体替换 `o.value = expr` 仍走原 setData 路径',
    descriptionEn: 'In a method body, `o.value.field = expr` / `o.value[key] = expr` (writing a field of a ref-held object/array) is rewritten to `this.data.o.field = expr` **plus `this.setData({ o: this.data.o })`**; whole replacement `o.value = expr` still uses the original setData path',
    why: '小程序视图由 setData 驱动：嵌套写此前只改 this.data、**不调用 setData** → 逻辑层数据已变但视图不重渲染（真机：p-checkbox 群选后计数/勾选视觉不变）',
    whyEn: 'The mini-program view is driven by setData: a nested write previously only mutated this.data without calling setData, so the logic layer changed while the view never re-rendered (real-device: p-checkbox group selection left the count/check visuals unchanged)',
    when: '方法/生命周期体内出现 `refName.value.<field> = ...` 或 `refName.value[<key>] = ...` 形态',
    example: {
      before: 'picked.value[d.name] = true',
      after: 'this.data.picked[d.name] = true; this.setData({ picked: this.data.picked })',
    },
    verify: 'tests/mp-transform.test.ts（ref 对象嵌套写补 setData 用例）',
    source: 'packages/compiler/src/script.ts → ref 写入重写（嵌套写分支）',
    decision: 'SOP v2 批次 1（p-checkbox 群选无效真机 bug 修复，2026-09-13）',
  },
  {
