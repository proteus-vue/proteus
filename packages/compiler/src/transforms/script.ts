...
export const SCRIPT_RULES: TransformRule[] = [
  ...
  },
  {
    ...
    phase: 'script',
    status: 'implemented',
    title: 'import 剥离警告（单文件产物无模块系统）',
    description: 'setup 顶层 import 在 MP 单文件产物中被剥离——跨模块引用将 undefined；编译期警告并提示内联/框架 store 桥',
    why: '反黑盒（vue-compat Batch A，决策 #116）：import 剥离不再静默，产物未定义引用显式提示',
    when: '<script setup> 顶层出现 import 语句',
    example: { before: 'import { useV } from "./util"', after: '警告（引用将 undefined）+ 剥离' },
    verify: 'tests/vue-compat.test.ts import 用例',
    source: 'packages/compiler/src/script.ts → transformScriptToPage（import 检测）',
    decision: '#116',
  },
  {
