...
export const SCRIPT_RULES: TransformRule[] = [
  ...
  },
  {
    ...
    phase: 'script',
    status: 'implemented',
    title: '顶层副作用语句保留（★#494：零缩进调用注入 onLoad 最前）',
    titleEn: 'Top-level side-effect statements preserved (#494: zero-indent calls injected at the start of onLoad)',
    description: '零缩进顶层表达式语句（initAppConfig(x) / registerCapability(x) 等调用形态）按源码顺序注入 onLoad 最前（先于 runtimeInits——初始化先于读取）；此前这类语句被静默丢弃，initAppConfig 不执行则后续 getConfig 必炸（config-demo 白屏根因之二）',
    ...
    example: { before: 'initAppConfig(appConfig)', after: 'onLoad 最前：initAppConfig(appConfig)' },
    verify: 'tests/app-config-mp.test.ts（#494 用例）',
    source: 'packages/compiler/src/script.ts → extractTopLevelCalls',
    decision: '#494',
  },
  {
