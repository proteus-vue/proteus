...
export const TEMPLATE_RULES: TransformRule[] = [
  ...
  {
    id: 'template/svg-canvas',
    phase: 'template',
    status: 'implemented',
    title: '含形状变化动画的 SVG → p-svg-canvas 组件（离屏 canvas 逐帧绘制）',
    titleEn: 'SVG with shape-morph animation → p-svg-canvas component (offscreen canvas per-frame drawing)',
    description: 'SVG 含形状属性动画（cx/cy/r/d/stroke-dashoffset 等，CSS 无法表达）→ 编译期产出 SvgScene → 运行时 p-svg-canvas 组件用离屏 canvas 逐帧绘制 + 定时器驱动 + toDataURL 回传 image src',
    descriptionEn: 'An SVG with shape-attribute animation (cx/cy/r/d/stroke-dashoffset etc., inexpressible in CSS) → compile-time SvgScene → runtime p-svg-canvas component draws per frame on an offscreen canvas + timer-driven + toDataURL back to image src',
    ...
    whyEn: 'G-62 animation: <image> is statically rasterized (internal animation does not play) and CSS only covers whole transforms, so shape-morph animations need per-frame redrawing; the offscreen canvas works (createPath2D accepts SVG d / rAF 62fps / toDataURL 2ms) while the visible canvas node() is unavailable',
    when: 'SVG 含 <animate attributeName="cx|cy|r|d|stroke-dashoffset|...">（形状属性）',
    example: { before: '<svg><circle cx="20"><animate attributeName="cx" values="20;80;20"/></circle></svg>', after: '<p-svg-canvas scene="{{proteusSvgScene1}}" />（离屏 canvas 逐帧）' },
    verify: 'tests/svg-canvas.test.ts',
    source: 'packages/compiler/src/svg-lower.ts lowerSvgToScene + template.ts（svg-canvas 分支）+ src/components/p-svg-canvas/',
    decision: 'G-62 Canvas 通道',
  },
  {
    id: 'template/svg-anim-promote',
    phase: 'template',
    status: 'implemented',
    title: 'SVG 整体变换动画 → CSS @keyframes（作用于 image）',
    titleEn: 'SVG whole-transform animation → CSS @keyframes (applied to image)',
    description: 'SVG 内部 <animateTransform rotate/scale/translate> 与 <animate opacity> → CSS @keyframes + 类名绑定 <image>；形状属性动画（cx/d/stroke-dashoffset）无法用 CSS 表达 → 不转译',
    descriptionEn: 'SVG-internal <animateTransform rotate/scale/translate> and <animate opacity> → CSS @keyframes + class bound to <image>; shape-attribute animations (cx/d/stroke-dashoffset) cannot be expressed in CSS → not translated',
    why: 'G-62 动画：真机实证 SVG 内部动画不播放（image 静态光栅化，§11.1），但 CSS 动画作用于 image 元素完全有效（三帧 MD5 各异）——整体变换类自动转译，形状变化类诚实不转',
    whyEn: 'G-62 animation: on-device evidence shows SVG-internal animation does not play (image is statically rasterized, §11.1), but CSS animation on the image element works fully (three frames with distinct MD5) — whole-transform animations are auto-translated, shape-morph ones are honestly skipped',
    when: 'SVG 含 <animateTransform rotate/scale/translate> 或 <animate attributeName="opacity">',
    example: { before: '<animateTransform type="rotate" from="0 50 50" to="360 50 50" dur="2s"/>', after: '<image class="proteus-svg-anim-1"/> + .proteus-svg-anim-1 { animation: … } + @keyframes' },
    verify: 'tests/svg-anim.test.ts',
    source: 'packages/compiler/src/svg-lower.ts collectSvgAnims + template.ts svgAnimToCss + index.ts（animCss → wxss）',
    decision: 'G-62 动画转译',
  },
  {
    id: 'template/svg-text-promote',
    phase: 'template',
    status: 'implemented',
    title: 'SVG <text> → 原生 <text> 叠加层（补 Skyline 丢弃文字短板）',
    titleEn: 'SVG <text> → native <text> overlay (works around Skyline dropping SVG text)',
    description: '静态 SVG <text>（含 tspan 拼接）编译期提取 → <view> 容器 + <image> + 绝对定位原生 <text>（viewBox 坐标 → 百分比、text-anchor → translateX、font-size/fill/font-weight 映射）；文字可选中、字体可控、随容器缩放',
    descriptionEn: 'A static SVG <text> (tspan concatenated) is extracted at compile time → <view> wrapper + <image> + absolutely positioned native <text> (viewBox coords → percentages, text-anchor → translateX, font-size/fill/font-weight mapped); text is selectable, font-controllable, and scales with the container',
    why: 'G-62 text 短板：真机实证 Skyline 解码 SVG 成功但丢弃文字元素（WebView 正常，官方文档未记载）——编译期提升为原生 text 是零运行时正解（对比 canvas fillText 需运行时绘制）',
    whyEn: 'G-62 text gap: on-device evidence shows Skyline decodes SVG successfully but drops text elements (WebView works; undocumented officially) — compile-time promotion to native text is the zero-runtime fix (vs canvas fillText which needs a runtime drawing pipeline)',
    when: '静态 SVG <text>（无 transform 父级——带 transform 诚实降级保留警告）',
    example: { before: '<svg><text x="50" y="25" text-anchor="middle">Hi</text></svg>', after: '<view style="position:relative"><image …/><text style="position:absolute;left:50%;top:25%;transform:translateX(-50%)">Hi</text></view>' },
    verify: 'tests/svg-to-image.test.ts',
    source: 'packages/compiler/src/svg-lower.ts collectTextNodes + template.ts svgTextToWxml',
    decision: 'G-62 text 提升',
  },
  {
    id: 'template/svg-hit',
    phase: 'template',
    status: 'implemented',
    title: 'SVG 图形事件命中（touchstart 坐标 + 几何判定）',
    titleEn: 'SVG shape event hit-testing (touchstart coordinates + geometric testing)',
    description: '静态 SVG 图形带 @click/@tap → 编译期收集几何（circle/rect/ellipse/path 包围盒）→ image 加 id + bindtouchstart → 运行时按 touches[0] 坐标换算 viewBox → 几何判定 → 调 handler',
    descriptionEn: 'A static SVG shape with @click/@tap → compile-time geometry collection (circle/rect/ellipse/path bbox) → image gets id + bindtouchstart → runtime converts touches[0] to viewBox coordinates → geometric test → calls the handler',
    ...
    whyEn: 'G-62 event hit-testing: on-device evidence shows Skyline tap events carry no coordinates (detail/touches undefined) while touchstart touches[0] has pageX/pageY, so hit-testing must use touchstart; although offscreen canvas getImageData/isPointInPath work, pure geometric testing has zero runtime dependencies and is lighter',
    when: '静态 SVG 图形带 @click/@tap（无 transform——transform 子树诚实降级不参与）',
    example: { before: '<svg><circle cx="30" cy="30" r="20" @click="onDot"/></svg>', after: '<image id="proteus-svg-hit-1" bindtouchstart="proteusSvgHit1"/> + 命中方法' },
    verify: 'tests/svg-hit.test.ts',
    source: 'packages/compiler/src/svg-lower.ts collectHitShapes + template.ts（svg-hit 分支）+ script.ts（proteusSvgHitN 生成）',
    decision: 'G-62 事件命中',
  },
  {
