...
export const TEMPLATE_RULES: TransformRule[] = [
  ...
  },
  {
    ...
    phase: 'template',
    status: 'implemented',
    title: 'v-model → 双向绑定编译（input 走 value+bindinput；★#500 自定义组件走 prop + update:arg 事件）',
    titleEn: 'v-model → two-way binding compilation (input → value+bindinput; ★#500 custom components → prop + update:arg event)',
    description: 'input/textarea 的 v-model="x" → value="{{x}}" + bindinput="proteusOnXInput"；★#500 自定义组件 v-model[:arg]="x"（p-modal v-model:visible）→ {{arg}}="{{x}}" + bind:update:arg="proteusUpdateArgModel"（页面 setData 回写 e.detail）——旧产物无脑 bindinput → 组件双向绑定永不生效（点击无反应真机根因）',
    ...
    whyEn: 'Mini Programs have no v-model syntax; the two halves of two-way binding are needed: a value/prop binding + an event write-back (script/vmodel-handler / vModelComponentHandlers); ★#500 v-model on Vue components is core semantics (prop + update:arg event contract) and must be compiled per spec',
