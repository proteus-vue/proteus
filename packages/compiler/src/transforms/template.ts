...
export const TEMPLATE_RULES: TransformRule[] = [
  ...
  },
  {
    ...
    phase: 'template',
    status: 'implemented',
    title: '作用域插槽警告（MP 无对等）',
    description: '<slot :item="x"> 作用域插槽在小程序无对等机制（父侧拿不到子数据）——编译期警告，建议 props 传子 + 事件回调传数据',
    why: '反黑盒（vue-compat-advance Batch 1，决策 #117）：不再静默输出无效属性',
    when: '<slot> 元素上有 v-bind 属性（非 class/style/name）',
    example: { before: '<slot :item="item" />', after: '<slot /> + 警告（MP 不传数据）' },
  ...
  },
  {
    ...
    phase: 'template',
    status: 'implemented',
    title: '<transition> → 框架内置转场组件（显隐动画）',
    description: '<transition name="fade"> 编译为框架组件（src/components/transition）：子元素 v-if 提取为 visible 绑定，组件内 CSS keyframes 播放进入/离开动画（fade/slide-up/scale）',
    why: 'Vue <transition> 运行时等价（vue-compat-advance Batch 2，决策 #117）：MP 无原生 Transition，框架组件 + visible 驱动补位；与路由 routeType 转场互补（元素级 vs 页面级）',
    when: '模板出现 <transition> 标签（单元素包裹 + v-if）',
    example: { before: '<transition name="fade"><view v-if="on">X</view></transition>', after: '<transition name="fade" visible="{{on}}"><view>X</view></transition>' },
    verify: 'tests/vue-compat-advance.test.ts Transition 用例',
    source: 'packages/compiler/src/template.ts → serializeElement（transition 分支）+ src/components/transition/index.vue',
    decision: '#117',
  },
  {
