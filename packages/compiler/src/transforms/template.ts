...
export const TEMPLATE_RULES: TransformRule[] = [
  ...
  {
    id: 'template/teleport-inline',
    phase: 'template',
    status: 'implemented',
    title: '<teleport> 解壳内联（无 root-portal 层叠，诚实警告）',
    titleEn: '<teleport> inline-stripped (no root-portal layering, honest warning)',
    description: '<teleport> 在小程序无 root-portal 层叠语义——解壳内联子元素（内容渲染但无层叠/传送）+ 诚实警告；Skyline root-portal 层叠为后续批次；此前原样输出无效 <teleport> 标签不渲染（比解壳更糟）',
    descriptionEn: '<teleport> has no root-portal layering semantics in Mini Programs — its children are inlined (content renders but without layering/teleport) with an honest warning; Skyline root-portal layering is a later batch; previously the invalid <teleport> tag was output verbatim and did not render (worse than inlining)',
    why: '小程序无 root-portal（Vue <teleport> 层叠/传送到 body 的对等）——诚实：内容内联可见 + 显式警告（反黑盒），弹层请用 p-popup/p-modal',
    whyEn: 'Mini Programs have no root-portal (the Vue <teleport> layering/teleport-to-body counterpart) — honest: content is inlined and visible + explicit warning (anti-black-box), use p-popup/p-modal for overlays',
    when: '模板出现 <teleport>（任意 target）时',
    example: { before: '<teleport to="body"><view>hi</view></teleport>', after: '<view>hi</view>（解壳内联 + 警告：无 layer-layer 语义）' },
    verify: 'tests/vue-compat-teleport.test.ts',
    source: 'packages/compiler/src/template.ts → serializeElement（teleport 解壳内联分支）',
    decision: 'teleport 诚实对齐（解壳内联）',
  },
  {
    id: 'directive/v-once',
    phase: 'template',
