...
export const STYLE_RULES: TransformRule[] = [
  ...
  },
  {
    ...
    description: '<style scoped> 存在时：:deep(X) 去包装 + 每条规则选择器末尾追加 .data-v-xxx（class 选择器；glass-easel 不支持属性选择器）；模板侧元素已附加 class="data-v-xxx"（template/scope-attr）',
    ...
    decision: '#77（v0.3 scoped CSS）+ Skyline 兼容修复（2026-08）',
  },
  {
