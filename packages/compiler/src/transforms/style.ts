...
export const STYLE_RULES: TransformRule[] = [
  ...
  {
    id: 'style/skyline-selector',
    phase: 'style',
    status: 'implemented',
    title: 'Skyline 不兼容选择器规则级剔除（★#495b）',
    titleEn: 'Rule-level removal of Skyline-incompatible selectors (#495b)',
    description: 'WXSS 产物中剔除含通配符 * 或 :deep 残留的选择器规则（Web 降级规则 .x-fallback > *、:deep(*) 编译后 .x *——微信 Skyline WXSS 编译器拒绝 token *，报 WXSS 文件编译错误）；Web 端保留（真浏览器可用）；剔除告警 + trace',
    descriptionEn: 'Drops selector rules containing the wildcard * or residual :deep from WXSS output (Web degradation rules like .x-fallback > * and :deep(*) compiling to .x * — the WeChat Skyline WXSS compiler rejects the * token with a WXSS compile error); the Web build keeps them (they work in real browsers); removal logs a warning plus trace',
    ...
    whyEn: 'Same source across targets: child selectors work in real browsers under the component slot model, while the Skyline WXSS compiler rejects wildcards — the same rule has different semantics per target, so removing it per target at compile time is the only zero-runtime approach (components degrade to plain containers on Skyline, G-22.2 plain-but-correct)',
    when: '选择器含通配符 * 或 :deep( 时（global/scoped 皆可——scoped 流程前剔除）',
    example: { before: '.p-grid-fallback > * { min-width: ... }', after: '该规则不出现在 wxss 产物 + 告警「WXSS 剔除 N 条 Skyline 不支持选择器规则」' },
    verify: '全量 build:mp WXSS 编译零报错（用户实测）',
    source: 'src/compiler/style.ts → transformStyleToWxss（skyline-selector 分支）',
  },
  {
    id: 'style/scoped-css',
    phase: 'style',
