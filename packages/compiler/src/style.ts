...
const BASE_SEMANTIC_WXSS = [
  // ★2026-08：text 行内语义（微信 text 语义；Skyline defaultDisplayBlock 会把所有节点 block 化）——
  //   类选择器 .proteus-text-inline（text 元素由 SEMANTIC_CLASS 附加该类；类选择器无 tag 隔离限制，
  //   tag 选择器方案需 tagNameStyleIsolation: legacy——当前开发者工具校验拒绝）
  '.proteus-text-inline { display: inline; }',
