...
// ★类型收口（10-type-consolidation）：公共类型定义已统一收口到 @proteus-vue/types/capabilities
// 本文件保留为 re-export 兼容层（包内 import './types' 与包外消费方路径不变）
// runtime 值（CapabilityError class）留本包（types 包保持纯类型）
import type { CapabilityErrorCode } from '@proteus-vue/types/capabilities'
...
export type {
  ...
  Capability,
} from '@proteus-vue/types/capabilities'
