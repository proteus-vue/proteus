<!-- showcase/subpackages/capabilities/pages/battery.vue —— 能力详情页（useBattery，官方形态）
...
const codeDemo = ref("const res = await useBattery()\nif (res.ok) { /* res.data: BatteryInfo { level, charging } */ }")

const out = ref('点击按钮读取电量（桌面浏览器常无 Battery API → 诚实降级）')
...
<template>
  <page-shell title="useBattery 电量" subtitle="能力原语 · capability.battery · 双端同源码">
    <demo-block index="01" title="真交互演示" :has-output="true" desc="同一份源码、同一个 Result&lt;T&gt; 契约——按 res.ok 分支，无回调、无 try/catch 义务" :code="codeDemo">
      <template #demo>
        <p-view id="demo-btns" class="btns">
          <p-button size="small" @click="onBattery">读取电量</p-button>
        </p-view>
      </template>
      <template #output>
