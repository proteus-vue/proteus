<!-- showcase/subpackages/capabilities/pages/download.vue —— 能力详情页（useDownload，官方形态）
...
const codeDemo = ref("const res = await useDownload(\"/assets/demo-fetch.json\", { responseType: \"json\" })\nif (res.ok) { /* res.data.status / res.data.data */ }")

const out = ref('点击按钮下载站内静态资源（真实 fetch，非模拟）')
...
<template>
  <page-shell title="useDownload 下载" subtitle="能力原语 · capability.download · 双端同源码">
    <demo-block index="01" title="真交互演示" :has-output="true" desc="同一份源码、同一个 Result&lt;T&gt; 契约——按 res.ok 分支，无回调、无 try/catch 义务" :code="codeDemo">
      <template #demo>
        <p-view id="demo-btns" class="btns">
          <p-button size="small" @click="onDownload">下载 JSON（真 fetch）</p-button>
        </p-view>
      </template>
      <template #output>
