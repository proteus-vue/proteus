<!-- showcase/subpackages/capabilities/pages/orientation.vue —— 能力详情页（useOrientation，官方形态）
...
const codeDemo = ref("const res = await useOrientation()\nif (res.ok) { /* res.data: OrientationInfo { angle, type } */ }")

const out = ref('点击按钮读取屏幕方向')
...
<template>
  <page-shell title="useOrientation 屏幕方向" subtitle="能力原语 · capability.orientation · 双端同源码">
    <demo-block index="01" title="真交互演示" :has-output="true" desc="同一份源码、同一个 Result&lt;T&gt; 契约——按 res.ok 分支，无回调、无 try/catch 义务" :code="codeDemo">
      <template #demo>
        <p-view class="btns">
          <p-button size="small" @click="onOrientation">读取方向</p-button>
        </p-view>
      </template>
      <template #output>
