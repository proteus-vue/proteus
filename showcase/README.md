...
subpackages/components/pages/<name>    组件详情页（每组件一页，官方形态）
  —— 进度 **73 / 73** ✅ **组件详情页已全部齐备**（目录页所有条目均可点击）
  —— 2026-09-24 批次 8（8 页，收官）：p-form / p-selection / p-keyboard-accessory
     / p-list-view / p-virtual-list / p-location / p-scan-qr / p-pick-photo（表单族 + 虚拟列表 + 能力入口）
...
├── subpackages/          ★分包（按大分类）
│   ├── components/pages/    组件详情页（73 页：全量齐备——p-button / p-stack / p-modal / …）
...
| **★主题皮肤用 `theme` 属性（编译器通道），不要写纯 CSS 皮肤包** | `theme="brand"` 经编译期落成组件根节点**单类变体**（`.p-theme--brand-<scope>`），变体样式定义在**组件自身 scoped wxss**（不跨边界 → 绕过样式隔离）。纯 CSS 皮肤包在 MP 端**物理不成立**（页面 wxss 够不到组件内部，即使 apply-shared）。主题键 SSOT = `src/components/theme/registry.ts`；回归锁 `tests/component-theme.test.ts` + E2E 计算色断言。★变体必须**单类选择器**（Skyline 不支持 `.a.b`）+ 只重定义局部变量 + 模板用**字面量键**（禁 `'p-theme--' + theme` 动态拼接）。详见 `docs/proteus-component-theme-plan/` |
| **★页面内按平台显隐用编译期宏 `__MP__`/`__WEB__`/`__TARGET__`** | 标准 `v-if="__MP__"` + 构建期宏 → 编译期静态裁剪**死分支不进产物**（替代 uni-app `#ifdef`，零新语法）。MP 端由 `compileVueSfc` 替换 + AST 静态裁剪；Web 端由 `platformMacroPlugin`（`enforce:'pre'` 源码替换，★vite define 对 .vue 不生效）。★script 内替换**跳过字符串字面量**（否则代码示例字符串被误改破坏引号）。回归锁 `tests/platform-macros.test.ts`，详见 `docs/proteus-platform-macros-plan/` |
| **★结构性平台差异用「平台变体文件」（工程架构基础层）** | 同一位置、不同平台不同文件：`foo.web.ts`/`foo.mp.ts`（业务代码）、`page.web.vue`/`page.mp.vue`（页面）、`logo.web.png`/`logo.mp.png`（public 资源）、`<route> platforms:[...]`（路由）、`<style src="./theme.css">`（CSS，第 5 层——`theme.web.css`/`theme.mp.css`）。构建期解析选一，**业务侧零平台名**；GS 式文件级分叉（非 #ifdef），差异可见、各自类型检查、死文件不进产物。★路由表 `component` 写**基准路径**（两端各自解析变体）；变体产物**去后缀**；扫描期**排除他端变体**。★两级平台模型：具体平台 `web|mp|ios|android|harmony` + 族 `native`（解析序：具体平台 → 族 → 基准），故 `s.ios.ts` 与 `s.native.ts` 都是合法后缀；别名 skyline→mp、app→native、native-ios→ios。演示页 `subpackages/components/pages/platform-variant`；回归锁 `tests/platform-variant.test.ts` + `tests/platform-variant-router.test.ts`；详见 `docs/proteus-platform-variant-plan/`。**分工：能拆文件的拆文件（默认）；拆不动的一个属性/一句话才用宏（兜底）** |
