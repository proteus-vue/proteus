...
describe('Vue 全能力基准线 SSOT（proteus-compiler-vue-align-plan）', () => {
  ...
  it('代表性能力状态（对齐框架既定基线）', () => {
    ...
    for (const p of ['onUpdated', 'onErrorCaptured', 'onBeforeUnmount', 'v-text', 'v-pre', 'v-once', 'readonly', 'defineSlots', 'defineComponent', 'defineModel', 'useModel']) {
    ...
    for (const u of ['getCurrentInstance', 'useSlots', 'useAttrs', 'h', 'createApp', '<component :is>', '自定义指令', 'isRef', 'isReactive', 'isProxy', 'isReadonly', 'isShallow', 'resolveComponent', 'renderSlot', 'mergeProps', 'toHandlers', 'withCtx', 'withScopeId', 'toRef', 'toRefs', 'toValue', 'unref', 'shallowReactive', 'shallowReadonly', 'defineOptions']) {
