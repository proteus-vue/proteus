...
describe('Vue 全能力基准线 SSOT（proteus-compiler-vue-align-plan）', () => {
  ...
  it('代表性能力状态（对齐框架既定基线）', () => {
    ...
    for (const a of ['ref', 'computed', 'watch', 'onMounted', 'defineProps', 'defineEmits', 'v-if', 'v-for', 'v-model', '<script setup>', '<style scoped>', 'provide', 'inject']) {
      expect(vueCompatStatus(a).status, `${a} 应为 aligned`).toBe('aligned')
    }
    // partial：语义受限（advance Batch 平台限制）——含 Step2 校准后降级项
    for (const p of ['defineModel', 'nextTick', 'onUpdated', 'onErrorCaptured', 'version', 'v-text', 'v-pre', 'v-once']) {
    ...
    for (const u of ['getCurrentInstance', 'useSlots', 'useAttrs', 'h', 'createApp', '<component :is>', '自定义指令', 'isRef', 'isReactive', 'isProxy', 'isReadonly', 'isShallow', 'resolveComponent', 'renderSlot', 'mergeProps', 'toHandlers', 'withCtx', 'withScopeId', 'defineComponent', 'withDefaults']) {
