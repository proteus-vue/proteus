...
describe('★mp-conformance 探针矩阵 P16：支持矩阵 fail-fast（#504 定格②——矩阵外语义硬报错）', () => {
  ...
  it('P16a：缺省软警告（四形态各自警告文案在位，不抛错）', () => {
    ...
    // ★2026-09-09 G-62 P0：静态 <svg> 已 lowering 为 <image>——用独立 SVG 子标签（<path>）验证 no-peer 警告
    const r3 = compileVueSfc(H + '<template><path d="M1 2"/></template>', { filename: 'p16a3.vue', ...opts })
    expect(r3.warnings.some((w: string) => /SVG 矢量标签/.test(w))).toBe(true)
  ...
  it('P16b：rules.failFast → 四形态编译期 CompilerError（fail-closed，消息带 fail-fast 标记）', () => {
    ...
    expect(() => compileVueSfc(H + '<template><path d="M1 2"/></template>', { filename: 'p16b3.vue', ...ff }))
