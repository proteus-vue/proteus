...
const COUNT_SNAPSHOT = { total: 89, template: 49, script: 26, style: 9, validate: 5 }
...
describe('★#505 M5 批 1：规则治理门禁——总数/分相快照 + verify 机器可解析', () => {
  it('规则总数与分相快照（89 = template 49 + script 26 + style 9 + validate 5——增删规则须同步更新本快照并跑 gen:reference）', () => {
    ...
    for (const [phase, n] of Object.entries({ template: 49, script: 26, style: 9, validate: 5 })) {
      expect(byPhase[phase], `phase ${phase} 规则数漂移（现 ${byPhase[phase] ?? 0}，快照 ${n}）`).toBe(n)
    }
  })

  it('字段齐全：全部规则 example.before/after + verify 非空（89/89 实测零缺失后固化——新规则登记必修）', () => {
    const rules = listTransformRules()
