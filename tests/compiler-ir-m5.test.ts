...
const COUNT_SNAPSHOT = { total: 105, template: 59, script: 32, style: 9, validate: 5 }
...
describe('★#505 M5 批 1：规则治理门禁——总数/分相快照 + verify 机器可解析', () => {
  it('规则总数与分相快照（105 = template 59 + script 32 + style 9 + validate 5——增删规则须同步更新本快照并跑 gen:reference）', () => {
    ...
    for (const [phase, n] of Object.entries({ template: 59, script: 32, style: 9, validate: 5 })) {
