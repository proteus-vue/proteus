...
4. **双语同步**：zh 改动同轮改 en 镜像（结构对齐：章节/表格/围栏），零中文残留收口；CI 跑 `check:en-drift`（★#493 结构签名漂移阻断——标题计数/逐表列×行/围栏数；确需时差豁免在 `website/scripts/check-en-drift.mjs` 白名单登记 file+check+reason，豁免失效自动提示回收）
