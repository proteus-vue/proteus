...
**状态**：✅ **B4 全部落地（数据层 #355 + 面板 UI #356）**

> **✅ B4 数据层（决策 #355）**：`@proteus-vue/render-backend` 新增 `ownership-observability.ts`（DevTools 所有权图数据层权威 TS 版）——**图 mutation 事件流**（`OwnershipGraph.subscribe` 动态订阅 + `OwnershipMutation`：register/state/edge，B1 图零破坏扩展）→ **① 历史时间线** `createOwnershipHistory(graph, { limit?, enabled? })`（alloc/drop/moved/borrow/weak/strong 记录含字节/源码行；环形缓冲裁剪 + clear/dispose；生产期 enabled=false 关闭）**② 计数器采样** `createOwnershipCounters(graph)`（V-07：O(1) 每资源、先快照存量再订阅、`consistent()` 与 stats 自证——V-01）**③ 四类检测** `diagnoseOwnershipIssues(graph, { destroyedScopes?, longBorrowMs=1000, now? })`（V-02 泄漏路径 = 已销毁 scope 仍存活资源 + 反向引用链 + 源码行 / V-03 长期借用 = borrows edge.since 距今超阈值（now 注入可测）/ V-04 跨页面强引用 = **GraphEdge.kind 新增 'strong'**（显式跨页强持有登记）且引用方 ≠ 资源 owner / V-05 无主资源 = owner null 存活节点）**④ alloc/drop 配对时间线** `buildOwnershipTimeline`（V-06：drop/moved 带 matchedAlloc 可点到源码行 + alive 未配对高亮，页面销毁 force-drop 后消失）+ `formatOwnershipDiagnosis` 可读报告（面板/CLI 展示契约）。测试 `tests/ownership-observability.test.ts` 11 用例（V-01 一致性×2（含图已有后挂载）/V-07 万级 alloc/drop 开销烟测 <2s + 历史关计数开/V-06 事件序+配对+裁剪+dispose/V-02~V-05 各检测/空诊断+报告）；全量 1785/173 无回归。

> **✅ B4 面板 UI（决策 #356）**：**devtools 第十视图 Ownership**（对齐 devtools-ownership-graph.md §2 主视图 + §4 面板）——`views/ownership.ts` `renderOwnership`（概要卡 alive/total/bytes + 四类告警区（🔴 无主/⚠️ 泄漏路径含引用链/🟡 长期借用/跨页强引用/✅ 无异常）+ 资源树（owner 分组 + 📍 源码行 + 🟡 借用方）+ 时间线（↑alloc/↓drop 配对 ↔ + 未配对 ⚠️ 高亮，视觉编码对齐 §2.2））；`ownership-info.ts` `createOwnershipTracer`（history+counters 常驻挂接 + collect 一次跑齐 JSON-safe 视图数据 + **存量快照**（面板后开不丢 alloc）+ dispose）；**双通道数据源**（本地面板 `ownershipData` 钩子 / 远程 **`Proteus.ownership`** WS 命令——ws-bridge 上报 + ws-source 缓存，与 deviceInfo 同构）；`installProteusDevtools({ ownership })`（缺省 true 挂 **`getProteusOwnershipGraph()` 全局惰性单例**——零配置可见，业务显式建图可传 { graph, history }）；V-01~V-07 数据全部上屏。测试 `tests/devtools-ownership-view.test.ts` 9 用例（渲染×2/tracer 采集×3（存量快照+B3 页面销毁联动+PageOwnership ctx）/panel 集成×2（钩子+空态）/WS 协议×2）+ dist-integrity 防回归（getProteusOwnershipGraph 单例不内联——#250 教训复用）+ 既有 nav/sends/重发断言同步（9→10/3→4/6→8）；全量 1794/174 无回归（17 unhandled 基线不变）。
...
> **✅ B5 已落地（决策 #357）**：**① PSS 三级配置解析**——`render-backend/src/pss.ts` `resolvePssMode(source, fallback?)`（文件头 pragma `@proteus-pss: strict|loose|off` 模块级声明——渐进采用 §7.1，未声明回退全局缺省 off）；**② PSS 限制检查 P1~P9**——`analyzePss(source, { mode?, ownedVars? })`：P1 禁 any/unknown / P2 禁动态属性写入（字符串字面量键放行）/ P3 禁 delete / P4 禁 eval·new Function / P5 禁 Owned 逃逸全局 / P7 禁闭包捕获 Owned / P8 禁 with / P9 禁原型链动态修改（**P6 跨页容器写入由 B-07 承接**——同一违规的规则面，避免双报）+ **CMP071 ref(Owned) 拦截**（ref/reactive/shallowRef/readonly 包裹 owned 变量 → error，message 指向 useOwned 替代）；PSS 分级（strict 全量 error / loose 仅 P1+P2 主路径 + CMP071 warning / off 跳过——文档 §2.2）；**③ 作用域级自动 drop 插入**——`insertScopeDrops(source)`：函数作用域内未处置 Owned（无 drop/transferTo）在函数闭合前插入 `<var>.drop()`（行前深度闭合判定；嵌套作用域各归其位；模块级声明不插——生命周期同应用；诚实边界：行级顺序流，提前 return 场景归 IR 级 transform）——**strict 验收「业务不写 drop 也能正确释放」**；**④ 管线组合**——`runPss(source, { mode?, autoDrop? })`：analyzeOwnershipSource（B-01~B-08）+ analyzePss + autoDrop + blocksBuild（strict + error → 阻断）——**编译管线独立步骤形态**（G-38 ProgramIR 只含模板语义树，script 源码检查在 parse 前调用——对齐 #333 dual-check 接线先例）；**⑤ B-07/B-08 规则补全**（borrow-checker 扩展）：B-07 跨页强引用（owned 写入 globalCache/store 赋值 + store.set 两形态 → G4007 error）/ B-08 循环引用（对象互指环 `a.x = b` + `b.y = a` → G4008 warning，自引用也算）；**⑥ useOwned/useBorrow（Vue 响应式集成）**——`@proteus-vue/api` 新增 `ownership-engineering.ts` `createOwnershipEngineering({ reactivity })`（注入式零 vue 依赖、结构类型 OwnedLike/BorrowLike 零硬依赖——对齐 createEngineering 先例）：`useOwned(owned)` 返回 `{ state: Ref<OwnedState>, byteSize: Ref<number>, borrow(), stop() }`（**只暴露元信息不暴露资源引用**——CMP071 替代）+ `useBorrow(owned)` 返回 `Ref<Borrow|undefined>`（**所有者释放时自动变 undefined**——Owned.subscribe 驱动）；render-backend `Owned` 加 `subscribe(cb)` 状态订阅（moved/dropped 通知——B1 零破坏扩展）。★语义固化：useBorrow 长持借用会阻止 transferTo（has_active_borrows——B-05 运行时表现，测试固化）。**测试**：`tests/pss.test.ts` 11 用例（pragma 解析×2 含超 20 行不生效/P 规则 strict 全量+loose 主路径+off 跳过/P5+P7 ownedVars 驱动/CMP071 双模式/insertScopeDrops 处置跳过+嵌套作用域+模块级不插/B-07 两形态/B-08 环+不阻断/runPss strict 组合+pragma 覆盖）+ `tests/ownership-engineering.test.ts` 8 用例（useOwned 元信息+drop/move 响应式+stop+borrow 委托/useBorrow drop 清空+dropped 不抛+transfer 拒绝固化/结构类型兼容）；全量 1813/176 无回归。
