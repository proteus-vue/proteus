...
- **G-36.8**：Agent 工具调用 **必须** 一步到位——**一旦陷入循环（连续输出「将执行 X」而无真实工具调用），立即停止并直接发出真正的 tool call**，禁止用文字重复描述下一步（经验教训：省 token + 防自旋死循环；工具调用是执行，思考描述不是执行）。
...
- **踩坑：Agent 陷入「输出循环」**（2026-09-08）：连续多轮仅输出「我要调用 X / 让me执行 Y」等思考文字，却未真正发出工具调用——表现为模型空转、token 白耗、用户等不到动作。**根因**：把「描述下一步」误当「执行」；工具调用必须写进 function_calls 块，思考文字不产生任何副作用。**教训**：① 所有动作一步到位经工具调用；② 一旦发现自己在重复描述将做什么而未调用工具，立即停止并只发一次真正的 tool call；③ 思考文字只用于真正的推理，不用于「预告动作」。（固化为 G-36.8）
