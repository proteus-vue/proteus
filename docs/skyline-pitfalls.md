...
| **S50** | 内置组件在 Web 端**完全无交互**（hover 无反馈 / 长按无菜单 / scroll 事件不触发） | 组件模板根节点写**原始 HTML 标签**（`<div>`/`<span>`/`<img>`）——Web 端保持原生元素，**Web 模拟层（proteus-view/text/image）不介入**（模拟层只改写小程序标签） | 组件模板用**小程序标签**（`<view>`/`<text>`/`<image>`/`<scroll-view>`）——Web 插件 `defaultScopedPlugin` 改写为 `proteus-*` → 复用模拟层；MP 端同为原生组件，产物不变 |
| **S51** | 小程序组件滚动监听里 **scrollTop 数字不变化**（或父级 `e.detail.x` 恒 undefined） | 组件把 MP 原生事件对象 `e = {detail:{...}}` **直接 emit** → 编译器 `triggerEvent(name, e)` **再包一层 detail** → 父级拿到 `e.detail.detail.x` | emit **裸载荷**：`emit('scroll', e?.detail ?? e)`（取内层 detail 再 emit）；父级统一读 `e.detail.x`（Web 模拟层直接发裸载荷，两端一致） |
| **S52** | 小程序组件 `<text>` 上写 `display:inline-flex` / 尺寸 / `transform` **全部失效**（图标不显示、旋转中心偏） | Skyline `<text>` 是**行内文本节点**，不支持 flex/盒模型/transform | 图标等「盒 + 字形」结构用 **`<view>` 盒**承载尺寸/居中/旋转（`transform-origin:50% 50%`），字形放内层 `<text>` |
| **S53** | `<image>` 承载 **URL-encoded 的 SVG data-URI 渲染成灰色方块** | Skyline `<image>` 仅完整渲染 **base64** 编码的 SVG data-URI（URL-encoded 形态真机落灰块） | SVG data-URI 一律 **`data:image/svg+xml;base64,<b64>`**（build 期 `Buffer.from(svg).toString('base64')`） |
| **S54** | 弹层**遮罩点击关不掉**（子组件 emit 了 update:show 但父级收不到） | 父级**手写 `@update:show`** → 编译器原样输出 `bind:update:show`（**双冒号**），而子组件 emit 的单段 `update-show`（glass-easel 事件名单段）→ **永不匹配** | 父级用 **`v-model:show`**，且编译器把 `@update:{arg}` **归一为 `update-{arg}`**（框架已修——`packages/compiler/src/template.ts`）；子组件 emit 单段 `update-{arg}` |
| **S55** | 横向滚动（scroll-x）子项**塌成一条线**（挤在一起） | ① 横向子项是**自定义组件**——Skyline 下自定义组件**宿主节点**在 flex 容器里布局不可靠；② 组件根 `.p-scroll-view{display:block}` 与页面 `.scroll-x{display:flex}` **同特异性争抢**（apply-shared 顺序不定） | 子项用**原生 `<view>`**；scroll-view 内**加一层 `<view>` 做 `display:flex;flex-direction:row`**（scroll-view 是滚动宿主，不是 flex 容器）；**组件根不声明 display**（布局语义归使用方） |
| **S56** | 受控属性（`scroll-top`）**复位无效**（手动滚动后点「回到顶部」不动） | `scroll-top` 是**受控**属性——只在值**变化**时驱动；手动滚动后变量仍是旧值 → 赋 0 无变化 → no-op | demo/使用方在 `@scroll` 里**回写**受控值（跟随真实位置），使复位产生真实变化（这是受控语义，非组件 bug） |
| **S57** | MP 端某 `ref` 绑定的值**恒为 undefined**（如 `data.img1` 空 → 图片不显示），Web 端却正常 | `ref(SOME_CONST)`——初值是**标识符**（非字面量），编译器**静态求值不出** → data 置 undefined（Web 编译期 inject 保留变量故正常） | 字面量**直接内联进 `ref()`**（`ref('data:...')`）；同类见 S33（`ref<T>(…)` 泛型 / `as` 断言）；编译器已告警「初值无法静态求值」 |
| **S58** | 横向 `scroll-view` 的**内层 flex wrapper 不撑开**（实测卡在容器宽 284px）→ 子项被裁、**内容不溢出** → 无横向可滚；只加 `white-space:nowrap` / `flex` 无效 | Skyline 下 `display:flex` 的块级容器默认**收缩到父宽**，子项超出也不撑开；`width:max-content` 不生效 | 内层 wrapper 用 **`display:inline-flex`**（随内容撑开，实测 284→738px）；★判据是**几何**（wrapper 宽 ≥ 内容跨度），不是「子项排成一行」——「一行但被夹住」同样滚不动 |
| **S59** | 受控 `scroll-top` 点「回到顶部」**只滚一点/回不到顶** | 编程滚动（`scroll-with-animation`）途中 `bindscroll` 把**中间值**（480/460…）回写成新 prop → 又成为新目标 → 动画被打断 | 编程滚动期间**暂停回写**（pending 目标值：设目标后 `bindscroll` 到目标才解除；期间忽略中间值）；★`onScroll` 里**不要**用 `setTimeout(()=>…, ms) as unknown as number`（编译器 ref/断言改写会产出非法 JS——实测「missing )」），用 pending 值而非定时器 |
| **S60** | 横向 `scroll-view` **渲染成一条细线**（内容看不见 / 无法滚动，滑动只让页面 top 微动） | 横向容器**只声明 `width`、无 `height`** → Skyline 下 scroll-view 交叉轴尺寸塌陷 → 容器塌成细线、子项被裁「看不到」（子项自身几何正常，故检查子项量不出问题） | 横向 scroll-view 必须声明**确定高度**（如 `height:56px`）；★与竖向需确定宽度对偶；**判据在「容器高度」而非「子项是否一行」** |
