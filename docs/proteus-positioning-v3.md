...
### 5.10 与主流框架的每一招：吸收什么、不绑定什么（2026-09-07 定稿）

**常被问到的"最终方向四招"——Svelte 式编译时优化、RN 的 JSI 通信、Flutter 的预渲染首帧、原生控件最终渲染——逐条对照官方方向（L0 规约原则 #0/#2/#3/#13.11 + G-48.3 + CMP028 + #505 决策），结论：每一招都被「语义层 + 可插拔后端」吸收为后端实现细节，框架不选任何一家为最终形态：**

| 外界提法 | 我们吸收的同源概念 | 判定 | 不绑定的边界 |
|---|---|---|---|
| Svelte 式编译时优化 | 原则 #3 编译期优先 + #505 IR-first（语义编译期定死、产物无运行时歧义、删规则即红）；MP 产物由编译器语义编译（ref/computed → data/setData 语义） | ⚠️ 吸收一半 | **不消灭运行时**：Web 端真 Vue（#505「不自研成熟工具链」）；MP 编译器注入 mini 响应式。Svelte 式"编译掉一切运行时"非目标（AOT 载体属 G-40 未来路径） |
| RN 的 JSI 通信 | G-40 Execution Carrier（#13.11 JSI 只是默认载体；G-40.4 >4KB 零拷贝 ArrayBuffer、G-40.5 commitBatch 批处理、G-40.6 载体可观测）；#13.12/G-40.3 实时能力逃原生线程 | ⚠️ 部分 | **小程序内唯一通道 = setData**（G-48.3 双线程语义不可绕过——平台约束非框架取舍）；JSI 直连只属自控宿主（G-41/42 App 容器/超级应用）的默认载体 |
| Flutter 预渲染首帧 | CMP028 首帧预算铁律（旗舰 16ms / 低端 33ms）；universal-backend Tier2「仅 SSR/预渲染输出」降级 | ❌ 基本不符 | Flutter 首帧快的根因是自绘引擎（A 路线）；G-27 明确不做自绘 vs 原生二选一（C 路线）。预渲染在我们这里是缺 JS 宿主的降级形态，非提速承诺——首帧走**预算达标**不是实现手法 |
| 原生控件最终渲染 | 原则 #2 语义收敛原生实现 + #10.8（语义原语须对应至少一个 OS 原生能力）+ G-31（Backend 实现"语义 → 原生控件"映射，非标签翻译） | ✅ 最接近 | 分层兑现：小程序 = 微信宿主自身原生控件体系（Skyline/glass-easel，我们管到产物语义正确）；App 三端原生控件 = conformance 原型态（真机接线在 App Renderer 批次）；白皮书 05 = 混合（详情 Native / 动效 Flutter / 大屏 Skia）——原生控件是主要后端而非唯一 |

**一句话对外口径**：Proteus 不做"四家拼盘选一"，而是让四家各自最擅长的一招都能作为后端实现细节接入同一份语义——**One semantic model. Any render engine.**（定位 v3 slogan）。

**诚实边界（G-37.8/G-56.8 数字不虚报）**：上表「JSI 载体」与「App 原生控件真机渲染」目前主要靠未来自控宿主 / App Renderer 批次兑现——native-* 后端是 conformance 原型（渲染控件名树），**尚无任一端完成"语义 IR → 原生控件"真机渲染验证**；对外只能宣称原型态。
