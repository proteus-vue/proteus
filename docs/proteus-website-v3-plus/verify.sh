...
# 六端齐全（HTML 用模板生成 data-k="${k}"，此处校验每个端标识 + 其拓扑关键词均出现）
for ent in "phone:悬浮 FAB" "tablet:左 240" "pc:固定侧栏" "car:焦点导航" "tv:横向海报" "watch:表冠"; do
  key="${ent%%:*}"; hint="${ent##*:}"
  if ! grep -qi "$key" "$FLEX"; then say "[FAIL] 缺端标识: $key"; exit 1; fi
  if ! grep -q "$hint" "$FLEX"; then say "[FAIL] 缺端拓扑证据($key): $hint"; exit 1; fi
done
# 核心语义契约（源码含 p-adaptive + touch 守卫）
if ! grep -q "p-adaptive" "$FLEX"; then say "[FAIL] 缺 p-adaptive 语义"; exit 1; fi
if ! grep -q 'v-if="touch"' "$FLEX"; then say "[FAIL] 缺 touch 守卫"; exit 1; fi
# 各端真实呈现形态（拓扑契约的关键证据 —— 关键词均取自文件真实用词）
for kw in "RECORDED ON DEVICE" "@conditional" "focus-tree" crown Skia Harmony AAOS; do
  if ! grep -q "$kw" "$FLEX"; then say "[FAIL] 缺关键词: $kw"; exit 1; fi
done
say "    6 端齐全 · p-adaptive · touch 守卫 · 拓扑关键词 全部在位"
