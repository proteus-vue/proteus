...
| p-popover MP 专项（2026-09-07） | dist/mp-weixin | ⚠️ 平台缺口待方案 | **探针实证**：触发链路全通（onTrigger FIRED → page data true）；但打开状态下**面板/layer 视觉不渲染**——skyline 对「fixed 兄弟 + absolute 锚定面板（非 fixed 根内）」不可靠（同 drawer v1 mask 族）；action-sheet/drawer/modal/popup 因面板在 fixed 容器内已正常，**popover 锚定悬浮需独立方案**（scene-layer 或 rect 定位或组件根 fixed 化，参考 p-modal popover 形态 Web-only 先例）→ 记录待专项，Web 不受影响 |
