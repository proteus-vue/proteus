...
| p-popover MP 专项（2026-09-07） | dist/mp-weixin | 🔶 推进中（里程碑 1 已修） | **探针实证**：触发链路全通；视觉不渲染。**里程碑 1 修复**：gen-routes 组件 json 漏 `componentFramework: glass-easel`（只给页面加）→ 组件内 root-portal 不渲染；补后组件内**常驻** portal 内容 ✅（V4）。**仍卡点**：portal 内容在 wx:if 内不渲染——疑 glass-easel wx:if 子树 × portal 限制，下一步试「portal 外层常驻 + visibility 切换」（对齐 p-drawer）；详见 semantic-primitives-plan/07 |
