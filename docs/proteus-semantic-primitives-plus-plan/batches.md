...
| **B3** | M2 | ⑤ 能力 50（Native Backend iOS/Android） | G-28 NativeBackend | ✅ **能力 Hook 层首期落地（`@proteus-vue/api/capability.ts`）**：CapResult&lt;T&gt; 契约（ok/err）+ createCapabilityHooks + CapabilityBridge（wx/web 双桥注入可单测）+ 10 个核心 useXxx Hook（location/vibrate/network/clipboard/screen/device/battery/orientation/share）+ probe() 降级探测——G-32.4 无回调/无全局对象/全类型/Result&lt;T&gt;；CMP007 门禁 `proteus api-check`（回调式 API/同步存储/裸全局调用 + 平台桥/兼容演示豁免）；剩余 40 能力待续 | 6 人月 |
