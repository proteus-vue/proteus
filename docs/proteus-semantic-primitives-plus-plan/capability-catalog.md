...
> 对应 G-32 §7 的 56 个能力原语（C1-C50 + ★C51 useUpdate / C52 useAlbum / C53 useWorker / C54 useAddress / C55 useWifi / C56 useWeRun），**逐一定义**：语义、参数、返回类型、降级行为、小程序原始 API 对照。  
...
> ② 矩阵引用一致性（`auditMatrixReferences`：`proteus` 列引用的组件/语义/Hook 必须真实存在，否则「幽灵引用」；
> `planned: true` 行诚实豁免 L2 规划）③ 闭环一致性。**报告区分「真·已落地」与「L2 规划待落地」**，避免「100% = 全做完」的误读。
