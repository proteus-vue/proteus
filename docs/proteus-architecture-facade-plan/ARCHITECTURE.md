...
| G-56 | Studio（自有宿主壳：第四宿主 + 移动端伴侣，内核零改动） | L4 工具链 | studio |
| G-57 | Inspector（三层可观测性叠加：出口协议化，叠加不替代） | L4 工具链 | inspector |
| G-58 | Plugin API（插件 API 与扩展生态：内置功能走同一 API + 零权限 + 版本化） | L4 工具链 | plugin-api |
| G-59 | Plugin Ecosystem（生态治理与性能契约：激活契约 + 数据分级 + 信任不可继承） | L4 工具链 | plugin-ecosystem |
| G-60 | Studio Website（官网落地：文档即契约 + 漂移阻断 + 版本可寻址） | L4 工具链 | studio-website-landing |

**关键路径**：G-01 → G-02 → G-03 → G-04 → G-07 → G-08 → G-10 → G-16/G-17 → G-18 → G-20；新增能力（G-21~G-60）各按其依赖插入，不阻塞原始主链。
