...
### ★linked 的**消费侧影响**（外部报告第 8 轮实测指出）

**外部工程不能再假设「所有包同一个版本号」**——各包 latest 会按需前进（本批实测：23 包 beta.11 / 12 包 beta.12 / 6 包 beta.13）。

| 做法 | 说明 |
|---|---|
| ✅ `npm view <pkg> dist-tags.latest` 逐包对齐 | 最准确 |
| ✅ 用 `^`/`~` 范围（如 `^0.3.0-beta.12`） | 注意 prerelease 的 caret 语义（元组需相同才匹配，见 check-package-health 的注释） |
| ✅ 本仓内部用 `workspace:*` | 天然指向工作区 |
| ❌ 写死某个统一版本号 | 会因某包没发到该版本而装不上 |
