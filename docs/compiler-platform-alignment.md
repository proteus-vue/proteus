...
| G5 | **v-model input 形态走 bindinput（组件半已修 G12-B）** | 87 文件产物普查：input → value+bindinput（#500）；组件 v-model → `bind:update-{arg}` 单段事件（G12-B 落地后，见 G12 行） | 官方 model:xxx = 引擎双绑标记（NormalAttributePrefix::Model + dash_to_camel，1.2/1.7⑤）+ **lvalue 实证**（官方 tests/tmpl/lvalue.test.ts）：`model:value`/`model:checked` 用于原生 textarea/input(checkbox)，`model:prop-a` 用于自定义组件——**子组件 setData 自身 property 即自动回写父 lvalue 路径**（comp.setData({propA}) → 父 a.b[c] 自动更新，无需任何 update 事件） | **登记观察（剩 input 半）**：组件半已迁单段事件（G12-B）；input 形态（bindinput）在 Skyline 实测可用（#500 系列）暂不改；候选 A（model:xxx）仍存远期官方对齐 | P1 | 🔶 设计专项（input 半观察；组件半 ✅ G12-B） |
...
| G12 | **双冒号事件名 bind:update:\*（v-model 组件契约载体）**——官方事件名单段标识符语法，双冒号 → InvalidAttributePrefix（Warn）+ 属性丢弃 = 事件不注册 | 87 文件产物普查 **13 种 p-* 组件 v-model 全部发射双冒号事件**（semantic-primitives-demo/fluid-system-demo 实证：bind:update:modelValue/visible/active/group/model-value）；**官方全仓零双冒号事件测试**（grep 实证：事件测试全部单段名 customEv 等）；**Skyline 真机三实证（2026-09-07 vmodel-mp-test）：p-modal 遮罩/✕关不掉、p-switch 无回传、p-input 不回传——而 WebView 全部正常 = 编译链丢弃双冒号事件** | 官方 tag.rs：parse_colon_separated 切三段 → 前缀 Invalid → Warn + 属性丢弃；事件合法形态 = bind:name 单段（含 `my-event` 连字符）；官方双绑不走事件（model:xxx + setData 回写，见 G5 lvalue 实证） | **✅ 候选 B 已落地（2026-09-07）**：事件名单段归一 `update:{arg}` → `update-{arg}`——页面侧 template.ts 契约发射 bind:update-{arg} + 组件侧 script.ts emit('update:X') 首参归一 triggerEvent('update-X')（非 update: 前缀不动）；IR vModelComponentHandlers.arg 语义保留；规则双语 + gen:reference；测试断言全更（13 处 + 新组件侧归一用例）；**真机重验通过（WebView + Skyline vmodel-mp-test：p-modal 关 / p-switch / p-input 回传全 ✓——p-input 系受控契约 :value+@input 非 v-model）**；A（model:xxx）存远期 | P1 | ✅ 已落地（候选 B，双端重验通过） |
...
2. **步骤 2（已执行 ✅ 2026-09-07，候选 B 落地）**：事件名归一 `update:{arg}` → `update-{arg}`（连字符单段——官方 ident 后续符含 '-'，测试 customEv 大写亦合法；比驼峰更贴合 wxml kebab 风格）；实施面 = script.ts 字面量归一 + template.ts 契约发射 + IR arg 语义保留 + 规则双语注记 + golden/87 重跑；真机重验 WebView + Skyline。
3. **A 存档为远期官方对齐候选**（Skyline 全面接管后 model: 是终局形态；届时 IR vModel 契约与组件协议一起迁，属独立专项）。

> ★执行记录（2026-09-07）：步骤 1 复测结论 = **Skyline 双冒号事件全丢实锤**（vmodel-mp-test：p-modal 关不掉 + p-switch 无回传 + p-input 不回传；WebView 全部正常）→ 步骤 2 候选 B 落地（差距表 G12 行 ✅）；待用户真机重验 WebView + Skyline（vmodel-mp-test 同页四形态）。
