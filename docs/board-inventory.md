...
| `proteus-host-integration-plan` | **G-41** | 🟡 **B1-B5 已落地（#342-#346）** | ★**宿主接入契约 + Vue 绑定架构（Host Integration）**：三方正交（框架核心 × 渲染引擎 × 宿主）+ **Vue 不变机制**（createRenderer + **nodeOps Dispatcher 方案 B**：全局转发 `currentBackend` → 热切换/混合渲染，切换 = 一次赋值）+ 职责边界 6 铁律 + 8 步接入流程 + host-conformance H-01~H-08（32 项）；**落地**：`@proteus-vue/render-backend` 的 `dispatcher.ts`（B1 转发层 + toIRNode + trace）/ `host-conformance.ts`（B2 32 项 PASS）/ `vue-bridge.ts`（B3 真实 Vue3 createRenderer 接入——SFC 落到 Headless/vue-dom/Flutter 任意后端）/ `web-host.ts`（B4 WebHostRuntime：Main + Worker + EventLoop）/ `hot-switch.ts`（B5 rebuild/rehydrate/hybrid 三策略）；铁律 G-41.1-6 + CMP051-058；★编号避让：原稿 G-38 与 compiler-backend-spi 撞号（决策 #341） |
| `proteus-host-container-plan` | **G-42** | 🟡 **B1-B5 已落地（#347-#351）** | ★**宿主容器契约 + 页面生命周期治理（Host Container）**：五层容器栈 + 六种容器策略（SinglePage/Stack/SuperApp/MiniProgram/Window/Embedded——不绑容器形态）+ **IR 单一 Owner + 五原子销毁** + 框架代管资源 + 超级应用沙箱/崩溃隔离/配额/安全网关 + **严禁 fork 铁律**；**落地**：`@proteus-vue/render-backend` 的 `container-spi.ts`（B1 SPI + 页面生命周期状态机）/ `stack-container.ts`（B2 StackContainer：页面栈 + 五原子 + 资源池 + LRU）/ `container-conformance.ts`（B3 C-01~C-08 38 项 + scanRepoForFork）/ `superapp-container.ts`（B4 沙箱 + 崩溃隔离 + 自动重启）/ cli `proteus conformance --repo`（B5 仓库治理 CLI + 权限网关）；铁律 G-42.1-6 + CMP059-066；★编号避让：原稿 G-39 与 host-runtime 撞号（决策 #341） |
| `proteus-ownership-plan` | **G-43** | 🟡 **B1-B4 已落地（#352-#355）** | ★**资源所有权模型 + 内存治理（Resource Ownership）**：Owned/Borrow/Weak/Managed 语义层（GC 管可达性，所有权管意图——治理 GC 盲区）+ **借用检查器分层保证**（PSS strict 编译期完备——吸收鸿蒙 ArkTS「限制换能力」/ loose 主路径 / off 运行时兑底）+ Drop 五阶段协议（与 G-42 五原子销毁对接）+ **DevTools 所有权图**（V-01~V-07）+ 跨设备所有权转移 `transferToDevice()`；**落地**：`@proteus-vue/render-backend` 的 `ownership.ts`（B1 Owned/Borrow/Weak/Managed + OwnershipGraph + QuotaTracker）/ `borrow-checker.ts`（B2 源码级静态分析 B-01~B-08 + PSS strict/loose/off）/ `page-ownership.ts`（B3 页面所有权上下文——**G-42 五原子第 3 步 releaseResources 委托 Drop 协议**：forceDrop 强制回收 + Managed 自动释放 + 配额兜底归零；StackContainer/SuperAppContainer `ownership` 接入 + `ownershipOf(pageId)`）/ `ownership-observability.ts`（B4 **DevTools 所有权图数据层**：graph mutation 事件流 → 历史时间线/计数器采样（V-07 O(1)）/四类检测（V-02 泄漏路径·V-03 长期借用·V-04 跨页强引用 strong 边·V-05 无主资源）/alloc-drop 配对（V-06）/报告契约）；铁律 G-43.1-6 + CMP067-073；★编号避让：原稿 G-40 与 execution-carrier 撞号（决策 #341） |
