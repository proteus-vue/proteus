...
| `proteus-svg-skyline-alignment-plan` | **G-62** | 📋 规划（2026-09-08 立项） | ★SVG → Skyline 对齐（SVG-Canvas 桥）：编译期 lowering（svg 子树 → MPSvgIR → mp-svg 组件）+ 运行时 Canvas 2D 重绘（path d 解析/gradient/clip/命中测试）+ CSS 折叠进 draw 参数；P0 静态图标 → P1 响应式矢量 → P2 mask/dash 动画；WebView 透传 svg、Skyline 走 canvas lowering；p-svg 组件 MP 侧汇合点 |
