...
| 16 | types-plan | ✅ 核心类型已落地（B3/B4/B5/B6 已落地；B7 待）| shims + 各包 IR 类型 + ProteusConfig TS + 平台守卫（铁律 #4）+ validateConfig（config:check）+ @proteus/types 独立包（Platform 共享类型 + JSON Schema + generate types --check）+ 超级应用加固（品牌类型/配置迁移/Schema Registry）| B7 测试矩阵 / Registry .d.ts codegen 完整化 |
