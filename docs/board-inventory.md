...
| `proteus-ai-agent-plan` | **G-36** | 🟡 **B1 已落地（#360）** | ★**G-23 的第一个具体落地**：AI Agent 接入——让 AI 自动产出符合柔性 IR 的标准业务代码（MCP Server 11 工具 + Agent Kit SDK + 4 Skill + 三层 Guardrails + 自修复循环 + Token 优化 + B1-B6）；**B1 落地**：新包 **`@proteus-vue/mcp`**（传输无关 MCP Server 核心——11 工具（含 search_primitives/get_design_token/validate_ir/run_conformance 六端/generate_code/write_file 双闸）/5 Resources/3 Prompts/CMP021 策略（write 双闸+限流+Schema 防注入+路径逃逸防护）；数据源全 SSOT 消费：G-32 原语目录/MP 对照矩阵/六引擎 capabilities 派生）；铁律 G-36.1-7 + CMP017-022；与 G-31 B6 迁移 / G-32 原语库 / G-37 RenderBackend SPI 协同；余 B2 Agent Kit SDK / B3-B6 Skill 与评测 |
