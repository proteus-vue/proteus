...
| `proteus-end-alignment-plan` | **—（G-31 属性级补充，不占新 G 号）** | 🟡 **M1 标尺机制 + M2 参考实现已落地** | ★**端能力对齐标准**：以**小程序为覆盖度标尺**（官方文档抓属性级清单，77 组件 / 793 属性）+ **Component IR 为对齐基座**（属性 = 语义约束，可归一/可合并），产出**三层绑定**（IR props ↔ 组件 props ↔ 原生属性）。机制：`scripts/gen-mp-component-attrs.mjs`（生成器）+ `scripts/audit-component-attrs.mjs`（审计：语义别名归一 + `--min` 棘轮）+ `tests/mp-component-attrs.test.ts`（门禁）+ `docs/generated/alignment-ratchet.json`（只增不减）。当前覆盖 **207/347 = 60%**（批次 1 高频表单 + 批次 2 容器与外壳全 100%；`p-button` 21/21 范式参考实现）。**后续所有端（iOS/Android/鸿蒙/Flutter）按 `06-new-end-playbook.md` 对齐** |
