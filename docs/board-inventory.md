...
| `proteus-host-integration-plan` | **G-41** | 📋 规划（已入库） | ★**宿主接入契约 + Vue 绑定架构（Host Integration）**：三方正交（框架核心 × 渲染引擎 × 宿主）+ **Vue 不变机制**（createRenderer + **nodeOps Dispatcher 方案 B**：全局转发 `currentBackend` → 热切换/混合渲染，切换 = 一次赋值）+ 职责边界 6 铁律（框架不碰原生/宿主不解析 IR/引擎不感知 Vue/业务无平台判断/业务不假设 JS/注册先于启动）+ 8 步接入流程 + host-conformance H-01~H-08（32 项）+ `host-reference.cjs`（双引擎同源 SFC 渲染验证：业务代码零改动）；铁律 G-41.1-6 + CMP051-058；★编号避让：原稿 G-38 与 compiler-backend-spi 撞号（决策 #341） |
| `proteus-host-container-plan` | **G-42** | 📋 规划（已入库） | ★**宿主容器契约 + 页面生命周期治理（Host Container）**：五层容器栈（进程/运行时/★容器/页面/视图）+ 六种容器策略（SinglePage/Stack/SuperApp/MiniProgram/Window/Embedded——不绑容器形态）+ **IR 单一 Owner + 五原子销毁**（unmount→unbindEvents→releaseResources→destroyIR→releaseQuota）+ 框架代管资源（pageContext.timer/on/request 页面销毁自动回收）+ 超级应用沙箱/崩溃隔离/配额/安全网关 + **严禁 fork 铁律**（scanRepoForFork 机器扫描，G-42.6）+ conformance C-01~C-08（38 项）+ `container-reference.cjs`；铁律 G-42.1-6 + CMP059-066；★编号避让：原稿 G-39 与 host-runtime 撞号（决策 #341） |
| `proteus-ownership-plan` | **G-43** | 📋 规划（已入库） | ★**资源所有权模型 + 内存治理（Resource Ownership）**：Owned/Borrow/Weak/Managed 语义层（GC 管可达性，所有权管意图——治理 GC 盲区：原生句柄/共享内存/定时器/流）+ **借用检查器分层保证**（PSS strict 编译期完备——吸收鸿蒙 ArkTS「限制换能力」/ PSS loose 主路径 / off 运行时兑底）+ Drop 五阶段协议（与 G-42 五原子销毁对接）+ **DevTools 所有权图**（可指出泄漏路径 + 源码位置）+ 跨设备所有权转移 `transferToDevice()`（Move 的分布式版本）+ conformance O/D/B/V/X（42 项）+ `ownership-reference.cjs`（33 项验证）；铁律 G-43.1-6 + CMP067-073；★编号避让：原稿 G-40 与 execution-carrier 撞号（决策 #341） |
