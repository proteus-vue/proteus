...
| `proteus-component-semantics-plan` | **G-31** | 🟡 **B1-B5 已落地 + B7 骨架** | ★**入口语义化**：C-IR schema + 属性约束（GRID_CONFLICT/CMP006）+ semantic 映射 + toComponentIR + **全后端消费 semantic（proteus-* 语义类）+ B5 conformance 门禁**（6 后端 × 6 fixtures 渲染快照一致，18 语义覆盖 ≥3 端）+ **B7 能力 Hook 层骨架**（@proteus-vue/api/capability.ts + CMP007 `api-check`）；G-31.1-4 + CMP005-8；**小程序组件集降级为 Layer 1 兼容层方向** |
