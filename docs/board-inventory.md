...
| plan | 编号 | 状态 | 说明 / 下一动作 |
|------|------|------|----------------|
| `proteus-architecture-facade-plan` | 规约层（执行序 G 表 + CI 门禁） | ✅ v3.18 | **`00-architecture.md`（全局执行序 G-01~G-60 + 9 铁律 + 包注册表）/ `ARCHITECTURE.md`（一页全景）/ `01-optimization-log.md`（v3.0~v3.18 变更）三文件**——由 `scripts/check-consistency.js` 校验「三文件 G 表同集合且 G-01~G-N 连续」（CI 门禁 `consistency.yml`）；v3.6（决策 #315）追加 G-36~G-39 四 SPI/Agent 行；v3.7（决策 #340）追加 G-40 execution-carrier 行；v3.8（决策 #341）追加 G-41/G-42/G-43 宿主层三 plan 行；v3.9（决策 #364）追加 G-44 testing-framework 行；v3.10（决策 #369）追加 G-45 dev-host 行；v3.11（决策 #385）追加 **G-46~G-52 七 plan 行**（resource-pool / combined-conformance / miniprogram-runtime / sandbox-isolation / developer-platform / test-ir-runner / cross-device-verification）+ 白皮书 `docs/proteus-whitepaper-plan/`（非 G 序，登记于「其他文档」）；v3.12（决策 #391）追加 **G-53 mobile-verification / G-54 devtools-suite 两行**（G-53=孤儿 plan 补登记，泛化序修正 15→17；G-54 原稿自编 G-55 编号避让重编、CMP-163~170→155~162）；v3.13（决策 #392）追加 **G-55 devtools-landing 行**（原稿自编 G-56 编号避让重编、原则 #13.60-62 撞号顺延 #13.63-65）；v3.14（决策 #393）追加 **G-56 studio 行**（原稿自编 G-57 编号避让重编、原则 #13.57-59 撞号顺延 #13.66-68）；v3.15（决策 #394）追加 **G-57 inspector 行**（原稿自编 G-58 编号避让重编、原则 #13.60-62 第三次撞号顺延 #13.69-71）；v3.16（决策 #395）追加 **G-58 plugin-api 行**（原稿自编 G-59——studio 预留号——编号避让重编、原则 #13.60-62 第四次撞号顺延 #13.72-74）；v3.17（决策 #396）追加 **G-59 plugin-ecosystem 行**（原稿自编 G-60 编号避让重编、原则 #13.60-64 第五次撞号顺延 #13.75-79）；v3.18（决策 #397）追加 **G-60 studio-website-landing 行**（原稿自编 G-61 编号避让重编、原则 #13.60-63 第六次撞号顺延 #13.80-83）；★新 plan 入仓必须同步本表（决策 #177/#315 纪律） |
...
| `docs/spi-first-methodology/` | **—（元方法论，不占 G 序）** | ✅ v1 入库（#373）+ 第 10~16 次登记（#385）+ 第 17~23 次登记（#391~#397） | **★SPI-First 五步法**：把「不绑 X」泛化（九次 → **二十三次**）抽象为通用工程方法论——① 找耦合点（grep 量化：出现 ≥2 文件 = 候选）→ ② 语义收敛（接口禁技术名词）→ ③ 可插拔后端（≥2 含 Mock/Headless——单后端 SPI 是假 SPI）→ ④ conformance（同一套测试跨后端可判定）→ ⑤ 诚实边界（性能/能力/适用三边界）；**二十三次泛化 ↔ 五步法映射表**（G-27/29/31/39/40/42/43/44/45 + G-46~G-60 逐行，G-55 为 G-54 落地不占序）+ 8 反模式 AP-01~08（单后端/泄名/无 conformance/绕接口/仅类型/过度设计/后端爆炸/成本不透明）+ 耦合点审计模板 + 速查卡 + verify.sh 自检 31 项 PASS；★不依附 Proteus 可独立使用；**新一次泛化须先在此登记方法论变更再落地 plan**（★第 10~16 次投影 G-46~G-52 已随决策 #385 补录登记；第 17~18 次投影 G-53/G-54 随决策 #391 登记——G-53 不绑设备供给、G-54 不绑 IDE 形态；第 19 次投影 G-56（自有宿主壳：不绑宿主来源）随决策 #393 登记；第 20 次投影 G-57（可观测性出口：不绑可观测性来源）随决策 #394 登记；第 21 次投影 G-58（插件 API：不绑扩展来源）随决策 #395 登记；第 22 次投影 G-59（生态治理：不绑生态治理模式）随决策 #396 登记；第 23 次投影 G-60（文档形态：文档即契约、renderer 非副本）随决策 #397 登记）；★原稿旧编号已重指向（宿主运行时 G-36→G-39、渲染 conformance G-34→G-27、载体 G-37→G-40、测试框架 G-41→G-44） |
| `docs/proteus-architecture.md`（规约） | 原则 #0-#13.83 + 铁律 + FLD/GLS/RND/NAT/PRIM/AI/CMP 规则 | ✅ **M1.1 已收口 + SPI 八系追加 + G-46~52 并入 + #391~#397 工具链七 plan 并入** | **真理来源**（原则 #0 统一语义收敛根 + 五支柱 + #13.x 可插拔可验证（含 #13.11-13.24 宿主层子原则 + #13.25-13.27 测试验证层 + #13.28-13.30 dev-host + #13.31-56 G-46~G-52 + #13.57-59 G-53 + #13.60-62 G-54 + #13.63-65 G-55 + #13.66-68 G-56 + #13.69-71 G-57 + #13.72-74 G-58 + #13.75-79 G-59 + #13.80-83 G-60）+ 分层/能力/落地三类铁律 + 严格规则 + 分层双路线 + 来源更新规则——合并 architecture-update + 决策链；G-37/38/39/40/41/42/43 SPI 铁律与 CMP017-073、G-44 铁律与 CMP074-081、G-45 铁律与 CMP082-088、G-46~G-52 铁律与 CMP089-146、G-53 与 CMP147-154、G-54 与 CMP155-162、G-55 与 CMP163-170、G-56 与 CMP171-178、G-57 与 CMP179-186、G-58 与 CMP187-194、G-59 与 CMP195-206、G-60 与 CMP207-227 已并入） |
...
| `proteus-semantic-primitives-plan` | **G-24** | 🟡 **B1-B4 全落地（#329/#337/#338/#339）** | 六大家族 + 原则 #10.8（须有系统原生对应才进核心 p-*）；B1 桌面交互原语（p-hover/p-shortcut/p-focus-trap/p-context-menu）；B2 系统集成四件套（p-notify/p-permission/p-clipboard/p-deeplink + v-p-permission 门禁）；B3 导航结构（p-master-detail/p-command ⌘K/p-tabs/p-breadcrumb）；**B4 生命周期/设备（p-lifecycle/p-state-restoration/p-network-status/p-low-power）** + **B5 网页原语四件套（决策 #449：p-scroll-observer / 跨窗消息 / 锚点定位 / 页面 URL——官网豁免回收）**——`@proteus-vue/desktop`（32 包）**21 模块 + 五 v-p-* 指令** Pure logic + Web 接线（§9 Device 家族由 @proteus-vue/api capability hooks 承接——防重复） |
| `proteus-device-adaptation-plan` | **G-25** | ⬜ 规划 | 三维断点 W×H×F（车机/TV/手表）；VEH001/TV001/WATCH001 |
| `proteus-svg-skyline-alignment-plan` | **G-62** | ✅ **收官（2026-09-10 真机复测通过；2026-09-11 全线收口）** | ★SVG → Skyline 四通道落地（静态 image data-URI / 动态 computed / 整体变换 CSS keyframes / 形状动画 p-svg-canvas Canvas 2D）+ P0-P2 全真机验证；`use`/`symbol` 编译期展开、`text` 提升原生、几何命中测试；诚实边界（v-for / 变换内文本 / 百分比单位）见 plan README §收官。**2026-09-11 Skyline 全线收口六批见下方「G-62+ 收口」行** |
| **★Skyline 全线收口**（2026-09-11，六批，用户「把 skyline 小程序的线整个收掉」） | — | ✅ **本轮落地** | **① 运行时判定 SSOT**（`@proteus-vue/shared/platform/runtime`）：detectRuntime（window 守卫，修 web wx 模拟层误判）+ detectMpRenderer（Skyline/WebView 区分）——接线 component-runtime / api / capabilities 三处漂移判定。**② `@proteus-vue/worklet`**（第 40 包）：官方 wx.worklet（shared/derived/timing/spring/Easing/runOnJS）+ applyAnimatedStyle 封装，非 Skyline 诚实降级。**③ 能力 API 对齐**：createCapabilityBridge 走 SSOT（修 web 误选 wx 窄桥）+ wx 桥三处假成功改显式错误（network/orientation/bluetooth）。**④ 组件真映射**：`@proteus-vue/fluid` createMpSizeObserverFactory + 组件层测量助手（SelectorQuery 驱动容器查询）——p-split/p-zone/p-toolbar 容器响应式在 Skyline 真生效（原恒堆叠/恒 sm/不折叠）；p-scroll/p-scrollable/p-draggable 静默降级改显式告警。**⑤ 语义一致性**：消除幽灵登记（p-box 补齐 / p-fluid 去 tag），补齐能力入口组件 p-scan-qr/p-pick-photo/p-location——TAG_SEMANTIC_MAP 65 项全有组件、audit coverage 100% 闭环。**⑥ 门禁加固**：components:audit 加豁免模型 + wx 别名/globalThis 绕过检测 + no-browser-observer 规则。 |
...
| `proteus-end-alignment-plan` | **—（G-31 属性级补充，不占新 G 号）** | 🟡 **M1 标尺机制 + M2 参考实现已落地（批次 1/2/3 全收口）** | ★**端能力对齐标准**：以**小程序为覆盖度标尺**（官方文档抓属性级清单，77 组件 / 771 属性）+ **Component IR 为对齐基座**（属性 = 语义约束，可归一/可合并），产出**三层绑定**（IR props ↔ 组件 props ↔ 原生属性）。机制：`scripts/gen-mp-component-attrs.mjs`（生成器，按官方**四类属性区块**采属性、剔除 marker/node 等**子对象 schema**）+ `scripts/audit-component-attrs.mjs`（审计：语义别名归一〔全局 + 按 tag 限定〕+ 棘轮阈值读 JSON）+ `tests/mp-component-attrs.test.ts`（门禁）+ `docs/generated/alignment-ratchet.json`（★单一事实源，只增不减）。当前覆盖 **294/325 = 90%**（批次 1 高频表单 / 批次 2 容器与外壳 / 批次 3 宿主能力 全 100%；`p-button` 21/21 范式参考实现）。**后续所有端（iOS/Android/鸿蒙/Flutter）按 `06-new-end-playbook.md` 对齐** |
| `proteus-semantic-primitives-plus-plan` | **G-32** | 🟡 **B1 ✅ + B2 ✅ + B4 Shell/Gesture + B3 Hook 五期** | ★**完整语义落地闭环 IR**：176 原语清单 SSOT（`PRIMITIVE_CATALOG`，#405 语义登记批 128→136 + ★C51 useUpdate → 137 + ★C2/C3 颗粒度对齐 → 142 + ★组件实例 API 对齐（canvas/element/intersection/media-query/video/audio/live-pusher/ad）→ 153 + ★权威标尺缺口（隐私/性能/预加载/图像编辑）→ 157 + ★权威标尺缺口批（E/F/G）→ 170 + ★组件批 H（selection/keyboard-accessory）→ 172 + ★组件批 I（camera；cover-view/cover-image 归 na——Skyline 同层渲染冗余）→ 173 + ★组件批 J（webview/ad/map；aria-component 归 na——ARIA 文档页）→ 176）+ SEMANTIC_ENUM 18→53→62 + `proteus audit coverage` 门禁 + **布局 12 + UI 18 + Shell 10 + Gesture 核心全双端落地（31 新组件 + `@proteus-vue/gesture` + v-gesture，audit 72 组件全过，54 implemented 语义）× 6 后端 conformance + ⑤ Capability Hook 五期（★81 能力原语全 Hook 化：一/二期 10 + useFetch/usePermission/useStorage + 三期 sensor/brightness/phone-call/biometric/payment/login/qr-code/useAuth + ★能力颗粒度对齐批次：蓝牙 BLE/文件系统/地图/相机/录音/传感器/NFC/通知/后台/日历/更新…）**；⑤ Capability 余 0（Hook 全就绪）/ ⑥ Engineering 28 待续 |
...
| `proteus-plugin-ecosystem-plan` | **G-59** | 📋 规划（已入库，#396） | ★**插件生态治理与性能契约（第 22 次泛化：不绑生态治理模式——首个治理形态轴）**：G-58 解决「插件怎么安全地跑」、G-59 解决「生态怎么活下来+跑得够快」：**激活时机是契约**（禁通配符、超预算拒绝加载非警告——个体理性导致集体劣化，硬约束内部化成本）+ **★权限建在数据敏感度上不建在 API 危险度上**（clipboard.writeText 能窃取助记词；有限集合建模收敛，未登记 API 保守按 secrets）+ **★信任不可继承**（代码哈希变化即撤销重授权——不检测恶意，切断信任继承假设）+ 去激活对称审计 + 版本并存不跟随 + 废弃必须提供替代 + 内置第三方同构（assertBuiltinParity 进 CI）+ **破坏率公开看板** + 数字三分类标注；铁律 G-59.1-9 + CMP195-206（含反模式断言与接缝命题扩展映射）+ INV-ECO-01~08 + AP-ECO-01~07；参考实现 self-test 78/78 + verify PASS=95；★编号避让：原稿 G-60→G-59、CMP-203~214→195~206、原则 #13.60-64 第五次撞号顺延 #13.75-79、泛化"第 19 次"→第 22 次；诚实边界：激活耗时为声明估算 / tier 映射仅代表性 API / 破坏率无真实历史数据 / **生态冷启动是运营问题架构无解** |
| `proteus-studio-website-landing-plan` | **G-60** | 📋 规划（已入库，#397） | ★**Studio 官网落地与插件 API 文档（第 23 次泛化：不绑文档形态）**——API 参考生成/指南手写/共享单一源三形态指向同一事实：**文档即契约（G-60.1 红线：可从 WIT 推导的内容一律生成，手写 review 直接拒绝）**+ 漂移即阻断不警告 + 未知版本返回 null 禁静默降级（SKIP≠PASS 同源）+ 版本无关内容单一源（复制 N 份=绑定 N 次）+ 废弃元数据驱动 + **★数字不粉饰（官网专项：性能数字必须标注目标/实测）**+ **★更新私钥不可恢复先备份再生成（全套体系唯一不可逆风险）**+ 删除页面等于制造 404（已移除 API 保留页标注 removed）+ **CMP 段位结构升级三段式**（INV-W1~8=207-214 + AP-W1~6=215-220 + 接缝 J1~J7=221-227——AP 与接缝命题正式入 CMP）；铁律 G-60.1-9 + CMP207-227 + INV-W1~8；参考实现 self-test 91/91 + verify PASS=131；★编号避让：原稿 G-61→G-60、CMP-215~235→207~227、原则 #13.60-63 第六次撞号顺延 #13.80-83、泛化"第 19 次"→第 23 次；诚实边界：INV-W2 属 CI 流程约束参考实现不可验 / WIT parser 未实现 / 未真实构建站点 / 性能预算未测 |
...
| `proteus-semantic-primitives-plan`（B1+ 落地产物） | G-24 | 🟡 B1-B5 | `@proteus-vue/desktop` 21 模块：p-hover/p-shortcut/p-focus-trap/p-context-menu + p-notify/p-permission/p-clipboard/p-deeplink + p-master-detail/p-command/p-tabs/p-breadcrumb + **p-lifecycle/p-state-restoration/p-network-status/p-low-power（决策 #339）** + **p-scroll-observer/跨窗消息/锚点定位/页面 URL（决策 #449 网页原语）** + v-p-permission 门禁 |
...
| **G-59.1-9** | 激活时机契约 / 去激活对称 / 版本并存 / 破坏率公开 / 权限锚点数据敏感度 / 信任不可继承 / 废弃提供替代 / 内置第三方同构 / 数字三分类 | plugin-ecosystem-plan |
| **CMP195-206** | 通配符+超预算拒绝（195）/ 去激活审计（196）/ 老版本可解析（197）/ 破坏率可度量（198）/ 数据分级（199）/ 内置不超额（200）/ 废弃无替代拒绝（201）/ 重授权（202）/ 反模式断言×3（203-205）/ 沙箱∧分级接缝（206） | plugin-ecosystem-plan |
| **G-60.1-9** | 文档 renderer 红线 / 漂移阻断 / 未知版本 null / 未命中不猜 / 单一源 / 元数据驱动 / 数字不粉饰 / 私钥备份 / 删除即 404 | studio-website-landing-plan |
| **CMP207-227** | WIT 生成漂移阻断（207）/ tag 触发（208）/ shared 单源（209）/ 横幅深链（210）/ noindex（211）/ 未知版本 null（212）/ diff 拦截破坏性（213）/ 缺签名拒分发（214）/ 反模式×6（215-220）/ 接缝×7（221-227） | studio-website-landing-plan |
