...
| **G-45.1-6** | 基座零插件知识 / 未装载调用 pending 非抛（装载后回放）/ 装载即验证（签名 + conformance 快检）/ 双层产物分离（base cacheKey 禁规模因子）/ 动态装载全链可观测 / 商店发布回静态链接（合规边界） | dev-host-plan |
