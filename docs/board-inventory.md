...
| 16 | types-plan | ✅ 核心类型已落地（B4 平台守卫已落地；B3/B5-B7 待）| shims（wx/Page/事件/类型提示全链路：PageOnLoad/RouteParamsByName/MpEvent）+ 各包 IR 类型（compiler/capabilities/api/router）+ ProteusConfig TS interface + **平台守卫**（getPlatform/matchPlatform/assertPlatform/exhaustiveCheck，铁律 #4 替代 #ifdef）| B3 codegen（独立 @proteus/types 包 + JSON Schema）/ B5 validateConfig / B6 超级应用加固 / B7 测试矩阵 |
