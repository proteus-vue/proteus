...
| 16 | types-plan | ✅ 全批收官（B1-B7）| shims + 各包 IR 类型 + ProteusConfig TS + 平台守卫（铁律 #4）+ validateConfig（config:check）+ @proteus/types 独立包（Platform 共享类型 + JSON Schema + generate types --check）+ 超级应用加固（品牌类型/配置迁移/Schema Registry）+ migrate types codemod（CI 门禁）| 独立 @proteus/types 包 Registry .d.ts codegen 深化（远期）|
