...
6. **第三方类型不重复造（§8-§9）**：小程序 `wx.*` / `App`/`Page`/`Component` 构造器参数 → 直接复用官方 `miniprogram-api-typings`（命名空间 `WechatMiniprogram`），Proteus 不自造；**仅 WXML 标签属性类型**官方不覆盖，由 Compiler 自建 `MpComponentSchema` 注册表。新增平台遵循同样原则（优先找官方 d.ts，缺则自建 schema）。
...
| **M8 小程序官方类型整合** | 接入 `miniprogram-api-typings`（`wx.*`/`App`/`Page`/`Component` 参数，命名空间 `WechatMiniprogram`）、自建 WXML 组件属性 `MpComponentSchema` 注册表、`MpSdkVersion` 基础库版本对齐、`mp/shims.d.ts` 临时补丁机制 | B8 |
| **M9 跨端 API 统一层** | 基于 `WechatMiniprogram` + Web `fetch` + App Native 类型，收敛为统一 `PlatformAPI` 接口；`wx.request`/`fetch`/`nativeFetch` 通过 `assertPlatform` 分派，业务层只依赖统一类型 | B9 |

> M1-M2 为 P0（B1-B2），**必须先行**；M3-M9 可后置但不得省略（M5 错误定位对齐 Compiler 的 source map 体系；M8-M9 确保小程序/Web/App 三端 API 类型统一且不重复造轮子）。
...
| `WechatMiniprogram.*`（官方 typings） | Platform / API plan（capability 后端直接复用） |
| `MpComponentSchema` / `MpComponentRegistry` | Compiler（SFC 模板属性校验）、Component plan（`p-*` 映射） |
| `MpSdkVersion` | CLI（config 校验）、Build（目标基础库版本） |
| `PlatformAPI`（M9） | API plan（跨端统一接口）、Component / Lifecycle 业务调用方 |
