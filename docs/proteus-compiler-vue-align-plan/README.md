...
> ★能力增强进行中（消除 unsupported/partial）：已转 aligned——`unref`/`toValue`（读取改写内联）、`isRef`（守卫内联，constSourceTypes 追踪）、`watchEffect`/`watchPostEffect`/`watchSyncEffect`（改写为 `watch(deps, cb, {immediate})`，deps 从 cb 提取 x.value）——黄金断言 + 全量绿。诚实结论：`isReactive/isReadonly/isProxy/isShallow` 因 MP 编译为普通 data（无代理/只读语义）保持 unsupported（不强行违背语义）。下一批候选：`toRef`/`toRefs`（引用提取）。
