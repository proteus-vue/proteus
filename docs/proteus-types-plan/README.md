...
> **★实现状态（2026-08）**：✅ 核心类型分散落地——shims（wx/Page/事件/类型提示全链路：PageOnLoad/RouteParamsByName/MpEvent）+ 各包 IR 类型（compiler/capabilities/api/router）+ ProteusConfig TS interface；**★B4 平台守卫已落地**（getPlatform/matchPlatform/assertPlatform/exhaustiveCheck，铁律 #4 替代 #ifdef，8 用例）；**★B5 validateConfig 已落地**（config:check CLI：错误码 CONFIG_MISSING_REQUIRED/CONFIG_INVALID_TYPE/CONFIG_INVALID_ENUM/CONFIG_UNKNOWN_FIELD + 字段路径定位，9 用例）；**B3/B6/B7 待**（codegen/超级应用加固/测试迁移）；类型诊断经 `proteus config:check`/`module:check`/`router:check`/`capabilities:check` 可定位。
