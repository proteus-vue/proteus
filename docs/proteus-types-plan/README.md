...
> **★实现状态（2026-08）**：✅ 部分实现——shims（wx/Page/事件/类型提示全链路：PageOnLoad/RouteParamsByName/MpEvent）+ 配置 schema（ProteusConfig）；B1-B2 已落地，B3-B7（IR 类型/平台守卫/校验器/超级应用加固）待；类型诊断经 `proteus module:check`/`router:check`/`capabilities:check` 可定位。
