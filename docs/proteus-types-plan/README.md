...
> **★实现状态（2026-08）**：✅ 核心类型分散落地——shims（wx/Page/事件/类型提示全链路：PageOnLoad/RouteParamsByName/MpEvent）+ 各包 IR 类型 + ProteusConfig TS interface；**★B4 平台守卫已落地**（铁律 #4，8 用例）；**★B5 validateConfig 已落地**（config:check CLI，9 用例）；**★B3 部分落地**（@proteus/types 独立包：Platform/PlatformTarget 共享类型 + ProteusConfig JSON Schema 单一来源 + proteus generate types 落盘/--check 防漂移，8 用例）；**B6/B7 待**（超级应用加固/测试迁移）；类型诊断经 `proteus config:check`/`generate types --check`/`module:check`/`router:check`/`capabilities:check` 可定位。
