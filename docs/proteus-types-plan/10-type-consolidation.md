...
| 类别 | 处置 | 说明 |
|------|------|------|
| 公共导出类型（interface/type/联合）| ✅ 移入 @proteus/types | CompileOptions/CompileResult/TransformRuleOverrides、RouteRecord/RouteMeta/NavigateOptions、CapabilityDefinition/CapabilityAPI、RequestConfig/RequestResponse、ProteusConfig、Platform/CapabilityPlatform/RouteTransition 等 |
...
```
@proteus/types（零依赖，纯类型 + schema 数据 + 品牌/迁移）
   ▲ compiler / capabilities / router / api / plugin-vite（type-only import + re-export）
```
- 各包 `types.ts` 改为：`export type { ... } from '@proteus/types'` + 本地保留 runtime 值
- type-only re-export → esbuild/tsc 擦除 → **MP 产物零运行时依赖**
- 各包 package.json 加 `@proteus/types` dependency（tsc 解析 + npm 安装）
...
- 各包 dist 产物无 @proteus/types runtime require（type-only 擦除验证）
