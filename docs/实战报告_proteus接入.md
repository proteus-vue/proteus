...
## 二十六、★ 第十二轮复测：`plugin-vite/cli beta.17`（2026-09-20）—— F-33 修复确认；新发现 F-34（store 前缀在指令里不剥离）

### 一、F-33 确认修复 ✅（含严格验证）

框架在 `da67f193` 修了**两处** MP 直出通道（我报告只指出共享模块一处，他们自己数出 `main.mp.ts → app.js` 通道同样有问题）：

| 通道 | 修法 |
|---|---|
| MP 共享模块（esbuild 直出） | `define` 合并 `cfg.vite.define`（用户 define 在框架白名单之后，语义同 vite 的 `{...framework, ...user}`） |
| `main.mp.ts` → `app.js`（esbuildTransform + 字符串拼接） | 新增 `__NAME__` → define 值的替换 |

**严格验证方法**（不只看"宏是否出现"，而是看"宏是否真的来自配置"）：

```bash
PROTEUS_API_BASE="http://127.0.0.1:9999" npm run build:mp
# 产物里出现 127.0.0.1:9999 ✅ ← 证明宏确实从配置注入（而非我加的兜底值）
npm run build:mp   # 恢复后产物回到 8760
```
产物里 `typeof __API_BASE__` 已被常量折叠为字面量（`let t="http://127.0.0.1:8760"; return t||...`）。

### 二、★ 顺带打通：小程序端首次真实连通后端

F-33 修好后，模拟器的**网络面板首次出现真实请求**（此前 `fetch` 在小程序不存在，静默失败）：

```
GET http://127.0.0.1:8760/api/health    → 200 · 10ms
GET http://127.0.0.1:8760/api/projects  → 200 · 74ms · 返回 3 个项目（宴者 15 / 狂臣 37 / 默者 47 章）
```

**但页面仍显示"还没有作品"** → 由此发现下一节的 F-34。

> **过程中修掉一个我方适配缺口**：原 `src/api/index.ts` 用浏览器全局 `fetch`，
> 而**小程序没有 `fetch`**（框架也不提供该名字的桥）——缺失时静默失败（构建通过、页面空态、网络面板零请求）。
> 已改用框架的统一平台入口 `createPlatformAPI()`（按平台自动适配：MP → `wx.request`，Web → `fetch`），
> 业务层零 `wx.*`，两端同一份代码。这是**官方推荐写法**（examples 的 `platform-api-demo` 同款）。

### 三、★ 新发现 F-34（major）：模板 `store.` 前缀**只在插值/`:prop` 里剥离，指令里不剥离**

**现象**：数据已取到（网络面板 200 + 完整 JSON），但视图不更新——产物里模板仍是 `{{store.projects}}`：

```html
<!-- dist/mp-weixin/pages/index.wxml（beta.17 产物） -->
<view wx:if="{{store.err}}">
<view wx:if="{{!store.apiReady}}">
<view wx:if="{{store.projects.length}}">
<view wx:for="{{store.projects}}">
```
而 `setData` 侧只展开了 `err`（`storeBindings` 只收集到它）——**两处用了不同的表达式通道，于是"数据来了但模板取不到"**。

**最小复现（同类问题的干净对照）**：

| 模板写法 | 产物 | 结果 |
|---|---|---|
| 插值 `{{ store.err }}` | `{{ err }}` | ✅ 前缀已剥离 |
| `:title="store.x"` | `{{ x }}` | ✅ |
| **`v-if="store.err"`** | **`{{store.err}}`** | ❌ 未剥离 |
| **`v-if="!store.apiReady"`** | **`{{!store.apiReady}}`** | ❌ |
| **`v-if="store.projects.length"`** | **`{{store.projects.length}}`** | ❌ |
| **`v-for="p in store.projects"`** | **`{{store.projects}}`** | ❌ |

**根因**（`packages/compiler/src/template.ts`）：
`rewriteStoreRefs()`（剥离 `store.` 前缀 + 收集顶层字段）只接在两个调用点——
**插值**（`:1706`）与 **`:prop` 动态属性**（`:1392`）；而
**`v-if`**（`:1192` 用 `exprContent(dir.exp)`）与 **`v-for`**（`:1208` 用 `f.list`）**走的是各自的表达式通道，均未调用它**。
→ 指令里的 `store.x` 既不剥离前缀、也不进 `storeBindings`（所以 `$subscribe` 的 setData 也不含这些字段）。

**影响**：任何用 store 驱动**条件渲染/列表渲染**的页面（= 绝大多数业务页面）在 MP 端都拿不到数据。
Web 端不受影响（走 Vue 运行时，无此编译期剥离）。

**建议**：把 `rewriteStoreRefs` 收敛为**表达式处理的唯一入口**（所有从指令/插值取表达式的地方都过它），
与 F-30/F-32 同一条纪律——**同一件事只允许一份实现**。

**我方规避（已实施）**：`index.vue` 的条件/列表改为**从 store 派生的 computed**（`projects` / `apiReady` / `errText`），
模板里用裸名（`v-if="projects.length"`）——即绕开该通道。其余页面同理待改。

### 四、本轮验证汇总

```
build:web        ✅ 2.73s
build:mp         ✅ 0 · app.js/app.wxss ✅ · WXML 全量合法
真机网络          ✅ 首次连通后端（health + projects 均 200，拿到 3 个项目数据）
真机视图          ⚠️ 未更新（根因 = F-34，已定位并有规避方案）
console          ⚠️ 仍有 1 条 RangeError（栈溢出——F-34 的连带：$subscribe 里 setData 字段与模板不匹配，
                      触发框架的响应式重算路径；待 F-34/规避完成后复测）
```

### 五、★ 端到端打通（本轮最终结果）

修完 F-33 与我方两处适配后，**小程序端首次完整跑通**：

```
真机网络    GET /api/health → 200 · 10ms ｜ GET /api/projects → 200 · 74ms（3 个项目）
真机视图    ✅ 宴者（15 章）· 狂臣（37 章）· 默者（Operator 005，47 章）全部正确渲染
真机 console ✅ 零错误（此前的 WXML 编译错误与 RangeError 均已消失）
产物        app.js ✅ / app.wxss ✅ / WXML 全量合法 / 框架组件按需输出 3 个
```

**至此 MP 端从"完全不可用"到"可构建 + 可渲染 + 可连通后端"**（追了 6 轮：beta.8 首次真跑 → beta.9/11/12 逐层解锁 → beta.15/17 打通）。
...
| 写法 | 后果 |
|---|---|
| `v-if="store.x"` / `v-for="p in store.items"` | ❌ 前缀不剥离 → 产物残留 → 取不到值 |
| 全改裸名 computed（`v-if="projects.length"`） | ❌ 框架收集不到 `store.xxx` → **不生成 `$subscribe`** → 数据到达后视图不更新 |

**最终解法（两条各取其长）**：
- **指令位置**（`v-if`/`v-for`）→ 用从 store 派生的 **computed 裸名**；
- **插值位置**（框架在此正确剥离并收集）→ 在**真实有意义的展示处**保留 `store.xxx` 引用
  （本工程：警示条 `{{ store.err }}`、列表头 `共 {{ store.projects.length }} 部作品`），
  使框架收集到 `err`/`apiReady`/`projects` 三个字段、生成完整 `$subscribe` 快照。

实测产物确认：`$subscribe` 已生成且字段齐全（`setData({ err, apiReady, projects })`）、
指令里无残留前缀、真机数据正确渲染。

> 另记一个**排除掉的弯路**：我先试过用 `<view v-if="false">{{ store.x }}</view>` 作"收集点"——
> 结果框架把 `v-if="false"` 的节点**静态裁剪**了（连收集一起省掉），`$subscribe` 变 0。
> 说明这类"隐藏收集点"的写法不可靠，必须在**真实展示位置**引用。

---

## 二十七、维护方处理回执（第 7~12 轮）

> 与前几份历史回执同理（第六/七/九/十/十一轮的单独回执此前未写，本轮一并补齐）：
> 回执内容**从 `docs/外部报告台账.json` 回溯**，使「已收口轮次必须有回执」对全部轮次成立。
>
> ★**本轮（第十二轮）两件必须先说清的事**（对应第七条）。
> 1. **F-34 是「修好了但没到你们手上」，不是修复无效**——你们复现残留时，工程解析到的是
>    `node_modules` 里**已发布的 `compiler@0.3.0-beta.15`**（其 dist 内只有 3 处 store 改写调用），
>    而修复当时还在框架工作树。把工作树产物换进去重建 → 你们产物 `store.` **零残留**；换回原包 → 残留复现。
> 2. **F-29（台账滞后于实际发布）在本轮修复过程中再次发生**——我核对台账时发现 F-27~F-32
>    六条仍标 `fix_state=worktree`，而 registry 实证它们早已发布。这是**同一失效模式第二次复发**
>    （第一次是你们报的 F-29 本身），性质上属我方流程缺陷，已补机器门禁（见下表格末行）。

### 第 7 轮（`0.3.0-beta.11` 复测 → 修复于 beta.12）

| # | 你们报的问题 | 修复版本 | 怎么验的 |
|---|---|---|---|
| F-27 | `v-model` 绑定对象属性/数组元素 → 产出非法 JS | `0.3.0-beta.12` | 你们独立复测通过（升级 `verified_by=external`）；框架侧新增路径安全 handler 名 + setData 键引号化 |
| F-28 | `v-model` 与 `@input` 同元素 → WXML `bindinput` 重复（DuplicatedAttribute） | `0.3.0-beta.12` | 同上；修法为 bindinput 合并 |

> **registry 实证（本轮补做）**：解包 `compiler@0.3.0-beta.12` grep 修复标记 `modelHandlerSuffix` → **已包含**。
> 此前台账标 worktree 属滞后，已更正为 `published`。

### 第 8 轮（`0.3.0-beta.12/13` 复测）

| # | 你们报的问题 | 状态 | 怎么验的 |
|---|---|---|---|
| F-29 | 台账滞后于实际发布（`fix_state` 仍标 worktree 而包已上 registry） | 已修（流程/门禁） | 新增 `pnpm sync:cobuild`（字段归属合并，不再人工复制）+ 三方门禁 `check:cobuild`；★**诚实记录：本轮复发，见开篇第 2 条** |

### 第 9 轮（真机启动失败 → F-30）

| # | 你们报的问题 | 修复版本 | 怎么验的 |
|---|---|---|---|
| F-30 | **阻断级**：框架组件按需输出失效——引用链经应用组件时，76 个 `p-*` 组件只产出 `index.json` → 真机启动失败 | `0.3.0-beta.14` | 你们独立复测：`usingComponents` 9 条映射缺失 0 条、四件套齐全；框架侧补应用组件目录 BFS + 声明与本体同源门禁 |

> **registry 实证**：`plugin-vite@0.3.0-beta.13` 无 `resolveAppComponentFile` / `beta.14` 有 → 确认 beta.14 已发布。

### 第 10 轮（F-30 确认修复 → 新报 F-31）

| # | 你们报的问题 | 修复版本 | 怎么验的 |
|---|---|---|---|
| F-31 | store 绑定依赖**未文档化的硬编码命名**（变量名必须叫 `store`、工厂名必须匹配 `use*Store`） | `0.3.0-beta.15` | 你们独立复测通过（本轮升级 `verified_by=external`）；修法：识别放宽到 `use\w*Store` 候选 + 未识别时显式告警，不再静默不绑定 |

### 第 11 轮（`0.3.0-beta.15` 复测 → F-32 确认、新报 F-33）

| # | 你们报的问题 | 修复版本 | 怎么验的 |
|---|---|---|---|
| F-32 | **黑屏根因**：WXML 三类违规未被构建期拦截（模板字面量 / `<template>` 子节点 / 非 WXML 标签） | `0.3.0-beta.15` | 你们独立复测：三类全部拦截 ✅；框架侧回归锁 + 标签映射对照权威清单（含一次自我纠错：不动 `label`/`audio` 原生组件） |
| F-33 | `vite.define` 在 MP 端不生效（产物残留未替换标识符）→ 运行时取值失败并触发栈溢出 | `0.3.0-beta.17` | 你们**严格验证**：改配置值重建 → 产物出现新值（证明真从配置注入，非兜底）；框架侧修**两条** MP 直出通道（你们当时只指出一处） |

### 第 12 轮（`plugin-vite/cli beta.17` 复测 → F-33 确认、新报 F-34）

| # | 谁报的 | 问题 | 状态 | 怎么验的 |
|---|---|---|---|---|
| F-33 | 你们 | `vite.define` 在 MP 端不生效 | ✅ 已确认修复（`published` / beta.17） | 你们的 strict 验证（`:9999` 注入） |
| F-34 | 你们 | 模板 `store.` 前缀只在插值/`:prop` 剥离，指令通道不剥离 | 已修（**worktree，待发布**） | **端到端**：工作树编译产物换入你们工程重建 → `dist/mp-weixin/**/*.wxml` 的 `store.` 零残留；还原原包 → 残留复现。回归锁 6 例、五通道逐条破坏性验证（撤掉任一通道 → 当场红） |
| F-35 | **框架自查**（同一次端到端验证中顺带发现） | 箭头事件处理器被「警告 + 原样输出」→ 产物 `bind:open="(n) => fn(n)"` 是**非法事件处理器名**（真机事件永不触发，只有一条 console 警告） | 已修（**worktree，待发布**） | 回归锁 5 例（含「产物不得含箭头函数」的反向断言）；破坏性验证：箭头分支返回 null → 4 例红 |

#### F-34 修法（回应你们指出的「同一件事只允许一份实现」）

你们在报告里给的判断是准确的：`rewriteStoreRefs()`（剥离前缀 + 收集字段）当时只接在**插值**与 **`:prop`** 两处，
`v-if`/`v-else-if`/`v-for`/`v-show`/`v-html` 各自走独立表达式通道、**都没过它**。
本轮把五条通道全部接上，**指令表达式不再有第二条路径**：

| 通道 | 修复后产物 |
|---|---|
| `v-if="store.err"` | `wx:if="{{err}}"` |
| `v-else-if="store.apiReady"` | `wx:elif="{{apiReady}}"` |
| `v-for="p in store.projects"` | `wx:for="{{projects}}"`（`wx:for-item="p"` 别名不误改） |
| `v-show="store.tree"` | `hidden="{{!tree}}"` |
| `v-html="store.html"` | `nodes="{{html}}"` |

且这些字段**同时进 `$subscribe`**（`setData({ err, apiReady, projects, tree, html })`）——
你们遇到的「两头堵」（指令要裸名 / 插值要留 `store.` 以生成订阅）**在修复后不再需要**：
指令里写 `store.x` 既可剥离、也照样被收集。

#### F-35 的成因（同族第三次，如实记录）

事件处理器「不是裸方法名」时的处理，历史上已有两次同类缺陷：
`#500`（赋值型 `x = !x` 原样输出 → 点击无反应）、`#505`（方法调用型补包装）。
箭头函数是**同一族的第三种形态**，当时只做了警告 + 原样输出，等于把「静默失效」降级成「有警告的静默失效」。
本轮补齐为包装方法，并明确**参数绑定随事件类型分派**：

| 写法 | 产物 handler | 生成方法 |
|---|---|---|
| 自定义组件事件 `@open="(n) => continueWriting(n)"` | `proteusInlineContinueWritingPayloadDet` | `this.continueWriting(e.detail)` |
| 原生事件 `@click="(e) => handle(e)"` | `proteusInlineHandlePayloadEvt` | `this.handle(e)` |
| store 方法 `@tap="(n) => store.open(n)"` | `proteusStoreOpenPayloadDet` | `this.store.open(e.detail)` |

不可校准形态（体里含 setup 局部变量 / 纯表达式 / 嵌套调用）**仍诚实警告并原样输出**，不静默改写。

> **你们工程里已命中此缺陷**：`src/pages/workbench.vue:161`
> `@open="(n) => continueWriting(n)" @write="(n) => continueWriting(n)"` ——
> 即「点击章节树节点跳转写作」在 MP 端此前是**不响应**的（被 F-34 的排查顺带带出来）。
> 这两个事件在 beta.15/17 上都不会触发，升级到含 F-35 的版本后才会生效。

### 本轮框架侧改动汇总

| 类别 | 内容 |
|---|---|
| 编译修复 | F-34（五通道接 `rewriteStoreRefs`）、F-35（箭头处理器包装 + 载荷分派） |
| 回归锁 | `tests/pinia-mp-compile.test.ts` +11 例（F-34 六例 / F-35 五例），全部破坏性验证 |
| 台账 | 条目 33 → 35；F-27~F-32 的 `fix_state` 按 registry 实证补正为 `published`；真实收口率 77% → **91%**（32/35） |
| 新门禁 | ① `ledger-check`：`fix_state=worktree` 必须声明 `packages`；② `release.mjs` 发布链末端：本次发布的包若覆盖仍标 worktree 的条目 → **逐条点名**（发布是翻状态的唯一时机，人工回想已两次失效） |
| 规则元数据 | `event/inline-expression` 规则描述/示例/验证同步更新（规则即文档 SSOT） |

### 待你们复测（升级后）

1. **F-34**：升级到含此修复的版本后，`v-if="store.x"` / `v-for="p in store.items"` 可**直接写 store 前缀**（无需 computed 裸名绕行），确认产物零残留 + 数据到达后视图更新。
2. **F-35**：`workbench.vue:161` 的章节树 `@open`/`@write` 应能真实跳转（这是本轮顺带修好的，你们可当新功能验）。
3. **F-29 复发**：本轮已加两道机器门禁，但「台账是否真的不再滞后」需要下次发布后由你们核对一轮——
   这也是这条流程类缺陷**唯一可信的验证方式**（框架自证不算数）。
