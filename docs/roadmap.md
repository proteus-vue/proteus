...
| 组件系统（✅ 已实现，决策 #79/#91） | `packages/compiler/` + `scripts/gen-routes.ts` | 已有：defineProps（对象形式 + TS 泛型）→ Component properties、defineEmits + emit() → triggerEvent、defineExpose（no-op 校验）、<slot> 透传、组件模式 attached 生命周期；父模板自定义标签 + 事件 bind: 冒号形式 + **usingComponents 自动注入 page.json**；TS 参数标注编译期剥离；待做：组件 usingComponents 嵌套（组件自身的组件依赖 json） |
