...
| 9 | **多端覆盖**：微信 / 支付宝 / 抖音 / 鸿蒙 / H5 | ✅（11 端） | ✅（12 端） | 🟡 微信 Skyline + Web | v0.5 起 |
...
MVP       工程化基线   编译能力补全  运行时与性能  多端扩展    生产可用    生态成熟
✅ 已完成  独立开源包  组件/派生    虚拟列表/状态  支付宝抖音  能力矩阵     devtools/插件
           CLI/脚手架  sourcemap    性能基准      鸿蒙/WebView 达标        组件库/社区
```
...
**目标**：证明 adapter + 映射表"可插拔"架构（决策 §0.3 原则 10），先覆盖国内小程序主流平台。
...
- **App 原生端（iOS/Android 原生渲染）**：非目标；H5 + 小程序矩阵满足主流诉求
- **自研 UI 组件库（初期）**：v2.0 前不提供，Web 直接复用 Vue 生态组件，MP 编译
- **全平台覆盖**：按 v0.5 列表扩展，不为"端数量"牺牲"端质量"
- **编译期为主原则不妥协**：任何"运行时 DOM 模拟"方案不在考虑范围（Taro 3 教训，决策 §0.3 原则 3）
...
| v1.0 | 能力矩阵全 ✅；真实项目验证报告；性能指标达标；文档站上线 |
| v2.0 | devtools 可用；首个社区组件库发布；插件体系文档化 |

---

**文档版本**：v2.48 · 本路线图随能力落地持续更新，每完成一个里程碑在 [PROJECT_MEMORY.md](../PROJECT_MEMORY.md) 归档决策。
