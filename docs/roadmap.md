...
> ★2026-09 架构方向定案（见 `docs/board-inventory.md` §1）：本文件的**版本线（v0.1→v2.0）** 与 `docs/proteus-roadmap-2-plan/` 的**里程碑线（M1-M3，L0-L5 六层）** 是同一规划的两套编排口径，映射如下：

| 版本线（本文档） | 里程碑线（roadmap-2-plan） | 分层 |
|------------------|---------------------------|------|
| v0.1 MVP（✅ 已完成） | M1 地基前序 | L2/L3 最小闭环 |
| v0.2 工程化基线（✅） | M1.2-M1.8（compiler/types/build） | L2 引擎 |
| v0.3 编译能力补全（✅） | M1.6-M1.7（style-safety / fluid） | L2/L1 |
| v0.4 运行时与性能（✅ 大部分） | M2.3-M2.5（fluid/adaptive/能力批量） | L3 能力 |
| v0.5 多端扩展（支付宝/抖音/鸿蒙） | M2.4（G-25 全终端） | L3/L5 |
| **v0.6 App 原生（自定义渲染器）** | **M1.4 + M2.1（G-27 SPI → NativeBackend/FlutterBackend）** | L1/L5 |
...
> 17 个规划板块的**权威状态总表**见 [docs/board-inventory.md](board-inventory.md)（2026-08 全仓盘点：12 ✅ / 1 🟡 / 4 ⬜）
