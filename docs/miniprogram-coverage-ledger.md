...
| 态 | 含义 | 计数 |
|---|---|---|
| ✅ **covered** | 有可运行等价（组件/语义/Hook；含改名承接：`batchGetStorageSync`→`useStorage`） | **228** |
| 📋 **planned** | L2 已声明待落地（诚实登记，**非已实现**） | **31** |
...
> **诚实指标**：`真·落地率 = covered / (covered+planned) = 228/259 = 88%`（可落地项中已可用的比例）。
