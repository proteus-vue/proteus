...
├── packages/shared/               # ★ @proteus/shared 公共层（平台适配器 + 全局类型声明）
├── packages/compiler/              # ★ @proteus/compiler 编译引擎独立包（v0.2 起 monorepo）
│   └── src/                        #   纯函数引擎 + transforms 规则注册表（69 条 AI 说明书，含 apply 分派层）
├── packages/runtime/               # ★ @proteus/runtime 运行时（setData 桥接 / 生命周期 / store 桥 / app 骨架）
├── packages/router/                # ★ @proteus/router 路由包（工厂化 createRouter + 守卫 + 预设 builders + 透明化规则）
├── packages/plugin-vite/           # ★ @proteus/plugin-vite Vite 插件（mp-weixin 编译管线 + gen-routes 路由生成器）
...
├── packages/api/                   # ★ @proteus/api 跨端 API 抽象（createApi 请求 + 设备信息 + createAuth 凭证托管）
├── packages/pinia-sync/            # ★ @proteus/pinia-sync 多端状态协同（LWW 零依赖默认 + CRDT 接口占位）
...
├── tests/                          # 522 个单元测试 + 8 个 Web e2e 测试
...
```bash
npm test                # 522 个单测（compiler / module / capabilities / api / runtime / router / plugin / cli / golden / pinia / storage / bundle-report / perf）
