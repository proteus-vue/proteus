...
// 这里 import.meta.glob 收集全部模块 → 按 frontmatter.order 排序 → 侧边栏/路由清单自动生成。
// 新增指南 = 放一个 md 文件（frontmatter 写 title/order），侧边栏零改动——「内容即数据」。
...
export const guides: GuideEntry[] = Object.entries(modules)
  .map(([file, mod]) => {
    ...
    return { slug, title, order, doc: mod.default }
  })
