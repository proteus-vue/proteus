...
const scenariosZh = [
  ...
  { icon: 'box', title: '设计系统与组件库', desc: '172 语义原语 SSOT 驱动 68 个语义组件 + 设计 token；布局语义编译期可校验，而非 CSS 事后救。' },
]
const scenariosEn = [
  ...
  { icon: 'box', title: 'Design systems & libraries', desc: '172 semantic primitives SSOT drive 68 semantic components + design tokens; layout semantics checked at compile time, not patched with CSS.' },
]
...
const capabilitiesZh = [
  ...
  },
  {
    tag: 'G-31/32',
    title: '语义原语 SSOT',
    desc: '172 语义原语单一事实源 → 68 个语义组件 → 50 implemented 语义 × 6 后端 conformance 门禁 + 81 Capability Hook。',
  },
  {
    tag: 'G-41/42/43',
    title: '宿主层三件套',
...
const capabilitiesEn = [
  ...
  { tag: 'G-31/32', title: 'Semantic primitives SSOT', desc: '172 semantic primitives SSOT → 68 semantic components → 50 implemented semantics × 6 backends under conformance gates + 81 capability Hooks.' },
...
const STATS_EN = [
  ...
  { value: '172', label: 'semantic primitives SSOT', source: 'PRIMITIVE_CATALOG (proteus audit coverage)' },
  { value: '50', label: 'implemented semantics × 6 backends', source: 'conformance gates' },
