...
export const CAP_SHARED_EN = {
  ...
  hMethods: '## Methods',
  hProps: '## Props',
  hTypeRefs: '## Referenced types',
  propCols4: '| Prop | Type | Required | Doc |',
  propSep4: '|---|---|---|---|',
...
export const CAP_METHODS_EN = {
  AuthState: {
    ...
    logout: 'Log out (clears the token)',
    setToken: 'Manually set the token (third-party login / server-issued session)',
    subscribe: 'Subscribe to auth-state changes (returns an unsubscribe function)',
  },
  WebSocketConnection: {
    send: 'Send a message (string or binary)',
    close: 'Close the connection',
    on: 'Subscribe to events (returns an unsubscribe function) — open/message/close/error',
  },
}

/** 用法示例 EN 覆盖（句柄型 hook 手写；其余由生成器默认 EN 分支模板 + CAP_ARGS_EN/CAP_DATA_HINTS_EN 生成） */
export const CAP_USAGE_EN = {
  ...
  generic: (call, hint, voidRet) => [
    `const res = await ${call}`,
    '',
    'if (res.ok) {',
    ...(hint ?? (voidRet ? ['  // success — void payload'] : ['  console.log(res.data)'])),
    "} else if (res.error.code.endsWith('.unsupported')) {",
    '  // platform unsupported → degradation path',
    '}',
  ],
...
/** 数据/void 型 hook 参数典型值（EN 页——与 zh CAP_ARGS 同构，含中文的值翻成 EN 防零中文破功） */
export const CAP_ARGS_EN = {
  useVibrate: '30',
  useShare: "{ title: 'Proteus framework', url: 'https://proteus-vue.cn' }",
  ...
  useCalendar: "{ title: 'Team sync', startTime: Date.now() + 3600000 }",
  useArchive: "{ src: 'wxfile://tmp/photo.jpg', quality: 80 }",
  useSMS: "'10086', 'Your verification code is 1234'",
  useFaceID: "'Verify it is you'",
  useInAppPurchase: "'com.example.premium'",
  useExtension: "'com.acme.exporter'",
}

/** 成功分支 data 用法行（EN——按返回类型给真实字段访问） */
export const CAP_DATA_HINTS_EN = {
  useLocation: ['  const { latitude, longitude } = res.data'],
  useNetwork: ["  console.log(res.data.online ? 'online' : 'offline', res.data.type)"],
  useClipboard: ['  console.log(\'clipboard:\', res.data)'],
  useQRCode: ['  console.log(\'scan result:\', res.data)'],
  useBiometric: ['  if (res.data) console.log(\'biometrics supported\')'],
  useBrightness: ['  console.log(\'brightness:\', res.data)'],
  useFaceID: ['  if (res.data) console.log(\'face authentication passed\')'],
  useContact: ['  console.log(\'contacts:\', res.data.map((c) => c.name))'],
  useLogin: ['  console.log(\'provider:\', res.data.provider, res.data.code ?? res.data.token ?? \'\')'],
  useUpload: ['  console.log(\'uploaded:\', res.data.status, res.data.progress ?? 100)'],
  useDownload: ['  console.log(\'downloaded:\', res.data.status, res.data.path ?? \'(blob/text)\')'],
  usePayment: ['  console.log(\'payment:\', res.data.provider, res.data.transactionId ?? \'\')'],
  useInAppPurchase: ['  console.log(\'iap:\', res.data.productId, res.data.state)'],
  useSensor: ['  console.log(res.data.kind, res.data.x, res.data.y, res.data.z)'],
  useNotification: ['  console.log(\'subscribe:\', res.data.templateId, \'granted:\', res.data.granted)'],
  useCamera: ['  console.log(\'camera:\', res.data.supported, \'granted:\', res.data.granted)'],
  useMicrophone: ['  console.log(\'microphone:\', res.data.supported, \'granted:\', res.data.granted)'],
  ...
  useBattery: ['  console.log(`battery ${(res.data.level * 100).toFixed(0)}%`, res.data.charging)'],
  useBluetooth: ['  console.log(\'bluetooth available:\', res.data.available)'],
  useNFC: ['  console.log(\'nfc:\', res.data.supported, res.data.available)'],
  usePermission: ['  console.log(res.data.permission, res.data.state)'],
  useEmbedded: ['  console.log(\'host:\', res.data.provider)'],
  useExtension: ['  console.log(\'extension returned:\', res.data)'],
}
