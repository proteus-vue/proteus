...
| code | 说明 |
|---|---|
| `phone-call.unsupported` | 桥未提供 makePhoneCall（usePhoneCall 不可用） |
| `phone-call.failed` | wx.makePhoneCall 失败 |
...
| 端 | 兼容 | 说明 |
|---|---|---|
| Web SPA | ⚠️ | vue-dom · webBridge 未提供 makePhoneCall → Err 显式降级（平台无直通 API） |
| 微信小程序 | ✅ | skyline（WebView 降级） · wx 桥 → wx.makePhoneCall |
