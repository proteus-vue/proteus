...
| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `extensionId` | `string` | 是 | 扩展/插件 ID（G-21 扩展点登记名） |
...
| code | 说明 |
|---|---|
| `extension.unsupported` | 桥未提供 loadExtension（useExtension 不可用） |
...
| 端 | 兼容 | 说明 |
|---|---|---|
| Web SPA | ⚠️ | vue-dom · webBridge 未提供 loadExtension → Err 显式降级（平台无直通 API） |
| 微信小程序 | ⚠️ | skyline（WebView 降级） · wx 桥未提供 loadExtension → Err 显式降级 |
