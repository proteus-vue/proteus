---
title: useWindow（capability.window）
group: 应用与生命周期
order: 7009
---

# useWindow

★C74 useWindow：窗口（wx.setWindowSize；web 缺省 → Err）

> 能力原语 C74 · `capability.window` · 返回 `WindowAPI` · **Hook 已实现**（API 就绪，双端桥见下表）

## 签名

```ts
useWindow(): CapResult<WindowAPI>
```

## 返回值

`Promise<CapResult<T>>`——铁律：无回调、无 try/catch 义务，`res.ok` 分支处理：

| 属性 | 类型 | 说明 |
|---|---|---|
| `ok` | `boolean` | 成功 `true` / 失败 `false` |
| `data` | `WindowAPI` | 成功载荷（方法结构见下） |
| `error` | `CapError` | 失败时存在：`code`（机器码）/ `message`（人读原因）/ `cause`（原始异常） |

## 方法

| 方法 | 签名 | 说明 |
|---|---|---|
| [`setSize`](#setsize) | `setSize(width: number, height: number): Promise<CapResult<void>>` | 调整窗口大小（PC 端）。 |

### `setSize`

```ts
setSize(width: number, height: number): Promise<CapResult<void>>
```

**说明**：调整窗口大小（PC 端）。

| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `width` | `number` | 是 | 宽（px） |
| `height` | `number` | 是 | 高（px） |

**返回值**：`Promise<CapResult<void>>`

## 错误码

| code | 说明 |
|---|---|
| `window.unsupported` | 桥未提供 getWindow（useWindow 不可用） |

> 平台不支持 → `*.unsupported` 族；业务按 code 分支处理，无需 try/catch。

## 兼容进度

| 端 | 兼容 | 说明 |
|---|---|---|
| Web SPA | ✅ | vue-dom · webBridge 实现（平台 API 直连） |
| 微信小程序 | ✅ | skyline（WebView 降级） · wx 桥 → wx.setWindowSize |
| Headless（SSR / 测试） | ✅ | headless · mock 桥注入（测试 / SSR 档） |
| iOS 原生 | 🟡 | native-ios（UIKit） · 端原型映射——能力桥未接线 |
| Android 原生 | 🟡 | native-android（Jetpack） · 端原型映射——能力桥未接线 |
| 鸿蒙 | 🟡 | native-harmony（ArkUI） · 端原型映射——能力桥未接线 |
| Flutter 混合 | 🟡 | flutter · 同一 JS 逻辑层——能力桥未接线 |
| 快应用 | ⬜ | 快应用引擎（待定） · 端未开始 |

> 状态口径：✅ 端已落地·本能力可用；⚠️ 端已落地·桥未提供→Err 显式降级；🟡 端原型映射·能力桥未接线；⬜ 端未开始。端架构对照见 [端与成熟度](/docs/framework/ends-matrix)。

> 铁律：能力原语全部返回 `Result<T>`（无回调 / 无全局对象）；平台不支持 → `Err` 显式降级，业务零平台分支。

## 用法

```ts
const win = useWindow() // 同步句柄——无 await、无 res.ok

if (win.ok) {
  await win.data.setSize(1280, 720) // PC 端调整窗口（需屏幕足够大）
} else if (win.error.code === 'window.unsupported') {
  // 非 PC 端 / Web 无标准 → 降级路径
}
```

<!-- generated by website/scripts/gen-content.mjs · 源码 SSOT：packages/component-ir/src/primitives.ts + packages/api/src/capability.ts -->