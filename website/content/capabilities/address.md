...
> 能力原语 C54 · `capability.address` · 返回 `ShippingAddress` · **Hook 已实现**（API 就绪，双端桥见下表）
...
| `data` | `ShippingAddress` | 成功载荷（结构见下） |
...
| `userName` | `string` | 是 | — |
| `postalCode` | `string` | 否 | — |
| `provinceName` | `string` | 是 | — |
| `cityName` | `string` | 是 | — |
| `countyName` | `string` | 是 | — |
| `detailInfo` | `string` | 是 | — |
| `nationalCode` | `string` | 否 | — |
| `telNumber` | `string` | 是 | — |
...
| code | 说明 |
|---|---|
| `address.unsupported` | 桥未提供 chooseAddress（useAddress 不可用） |
| `address.failed` | 选择收货地址失败 |
...
| 端 | 兼容 | 说明 |
|---|---|---|
| Web SPA | ⚠️ | vue-dom · webBridge 未提供 chooseAddress → Err 显式降级（平台无直通 API） |
| 微信小程序 | ✅ | skyline（WebView 降级） · wx 桥 → wx.chooseAddress |
