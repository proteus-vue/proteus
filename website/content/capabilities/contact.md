...
| code | 说明 |
|---|---|
| `contact.unsupported` | 桥未提供 chooseContact（useContact 不可用） |
| `contact.failed` | wx.chooseContact 失败 |
...
| 端 | 兼容 | 说明 |
|---|---|---|
| Web SPA | ⚠️ | vue-dom · webBridge 未提供 chooseContact → Err 显式降级（平台无直通 API） |
| 微信小程序 | ✅ | skyline（WebView 降级） · wx 桥 → wx.chooseContact |
