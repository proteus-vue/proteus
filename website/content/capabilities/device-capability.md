...
> 能力原语 C81 · `capability.device-capability` · 返回 `DeviceCapabilityAPI` · **Hook 已实现**（API 就绪，双端桥见下表）
...
| `data` | `DeviceCapabilityAPI` | 成功载荷（方法结构见下） |
...
| code | 说明 |
|---|---|
| `device-capability.unsupported` | 桥未提供 getDeviceCapability（useDeviceCapability 不可用） |
...
| 微信小程序 | ✅ | skyline（WebView 降级） · wx 桥 → wx.checkDeviceSupportHevc |
