...
> 能力原语 C56 · `capability.we-run` · 返回 `WeRunData` · **Hook 已实现**（API 就绪，双端桥见下表）
...
| 属性 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `encryptedData` | `string` | 是 | 加密的步数数据（wx.getWeRunData 返回的 cloudID/encryptedData；需业务后端解密） |
| `iv` | `string` | 是 | — |
| `cloudID` | `string` | 否 | cloudID（小程序云开发场景免解密） |

## 错误码

| code | 说明 |
|---|---|
| `werun.unsupported` | 桥未提供 getWeRunData（useWeRun 不可用） |
| `werun.failed` | 获取微信运动数据失败（需 scope.werun 授权 + 后端解密） |
...
| 端 | 兼容 | 说明 |
|---|---|---|
| Web SPA | ⚠️ | vue-dom · webBridge 未提供 getWeRunData → Err 显式降级（平台无直通 API） |
| 微信小程序 | ✅ | skyline（WebView 降级） · wx 桥 → wx.getWeRunData |
