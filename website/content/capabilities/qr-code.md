...
useQRCode：扫码（wx.scanCode；web 需摄像头取流源 → 降级 Err）
...
| code | 说明 |
|---|---|
| `qr-code.unsupported` | 桥未提供 scanQR（useQRCode 不可用） |
| `qr-code.failed` | wx.scanCode 失败 |
...
| 端 | 兼容 | 说明 |
|---|---|---|
| Web SPA | ⚠️ | vue-dom · webBridge 未提供 scanQR → Err 显式降级（平台无直通 API） |
| 微信小程序 | ✅ | skyline（WebView 降级） · wx 桥 → wx.scanCode + canvas |
