...
| 语义 | 域 | 小程序等价 |
|---|---|---|
| ui.image | 内容与表单 | `<image>`（L1 原语） · `<cover-image>`（L1 原语） |
...
| 微信小程序 | ✅ | skyline（WebView 降级） · 原生控件映射 → `<image>`（L1 原语） · `<cover-image>`（L1 原语） |
...
| `showMenuByLongpress` | 长按图片显示菜单（发送给朋友/保存/识别二维码等） | `Boolean` | `false` | 否 |
| `fadeIn` | 是否渐显（Web 用 CSS 淡入动画映射） | `Boolean` | `false` | 否 |
| `preload` | 是否预加载（设置 src 时即下载解码） | `Boolean` | `false` | 否 |
| `webp` | 是否解析 webP 格式（默认仅网络资源） | `Boolean` | `false` | 否 |
| `referrerPolicy` | ★官方 <cover-image> referrer-policy：请求的 referrer 策略（Web 原生 img 同名属性） | `String` | `''` | 否 |
...
- **类型**：`String`　**默认值**：`'aspectFill'`　**必填**：否
- **说明**：模式/裁剪方式（各组件枚举见类型列）
...
- **类型**：`Boolean`　**默认值**：`false`　**必填**：否
- **说明**：长按图片显示菜单（发送给朋友/保存/识别二维码等）

#### `fadeIn`

- **类型**：`Boolean`　**默认值**：`false`　**必填**：否
- **说明**：是否渐显（Web 用 CSS 淡入动画映射）

#### `preload`

- **类型**：`Boolean`　**默认值**：`false`　**必填**：否
- **说明**：是否预加载（设置 src 时即下载解码）

#### `webp`

- **类型**：`Boolean`　**默认值**：`false`　**必填**：否
- **说明**：是否解析 webP 格式（默认仅网络资源）

#### `referrerPolicy`

- **类型**：`String`　**默认值**：`''`　**必填**：否
- **说明**：★官方 <cover-image> referrer-policy：请求的 referrer 策略（Web 原生 img 同名属性）
...
```vue
<p-image :disabled="true" :src="'…'" :alt="'…'">
  <p-text>内容</p-text>
</p-image>
```

<!-- generated by website/scripts/gen-content.mjs · 源码 SSOT：packages/components/p-image/index.vue -->