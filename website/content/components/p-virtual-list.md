...
| 语义 | 域 | 小程序等价 |
|---|---|---|
| layout.virtual-list | 布局 | `<scroll-view>`（L1 原语） · `<list-view>`（L1 原语） · `<grid-view>`（L1 原语） |
...
| 微信小程序 | ✅ | skyline（WebView 降级） · 原生控件映射 → `<scroll-view>`（L1 原语） · `<list-view>`（L1 原语） · `<grid-view>`（L1 原语） |
...
<!-- generated by website/scripts/gen-content.mjs · 源码 SSOT：packages/components/p-virtual-list/index.vue -->