...
| 语义 | 域 | 小程序等价 |
|---|---|---|
| ui.select | 内容与表单 | `<picker>`（L1 原语） · `<selection>`（L1 原语） |
...
| 微信小程序 | ✅ | skyline（WebView 降级） · 原生控件映射 → `<picker>`（L1 原语） · `<selection>`（L1 原语） |
...
<!-- generated by website/scripts/gen-content.mjs · 源码 SSOT：packages/components/p-select/index.vue -->