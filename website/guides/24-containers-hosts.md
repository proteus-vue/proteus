...
> **容器管「页面怎么销毁」，所有权（G-43）管「资源归谁」**——两者合并才是完整的运行期内存治理（见[所有权工程](/docs/23-ownership)）。
...
宿主 × 引擎的每个组合按 **Tier** 声明并机器验证（Tier 1 = 承诺验证 / Tier 3 = 混入可行不承诺 / Tier 0 = 跨生态不合法），详见[一致性验证](/docs/26-conformance)。

## 下一步

- [一致性验证](/docs/26-conformance)：Tier 矩阵与 conformance 套件
- [路由](/docs/21-router)：页面栈之上的导航语义
- [所有权工程](/docs/23-ownership)：页面销毁如何做到不可泄漏
