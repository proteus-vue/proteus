...
- **AI Agent 的无设备回归**：Agent 改完代码不必起模拟器——语义快照 diff 即回归证据（见[AI 原生开发](/docs/08-ai-agent)）。

工程化接入（测试命令、部署链路）见[测试与部署](/docs/10-testing-deploy)；验证体系全貌（结构层 / 约束层 / 渲染层三层验证与门禁清单）见[一致性验证](/docs/26-conformance)。

## 下一步

- [渲染后端](/docs/04-render-backend)：Headless 在六后端中的位置
- [一致性验证](/docs/26-conformance)：语义快照如何变成门禁
- [测试与部署](/docs/10-testing-deploy)：接入 CI 工程链路
