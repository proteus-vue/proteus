...
一句话：**只学语义组件，不学各端**。心智模型只有一条公式——语义定义（框架做）+ 后端实现（平台做）：业务代码只消费语义接口（p-* 组件 + 53 个 Capability Hook），对渲染后端、编译后端、宿主零感知。已经会 Vue 3 SFC 的团队几乎没有新语法要学；方法论细节可从「统一语义收敛」五支柱读起，那是官方指定的 onboarding 第一课。
