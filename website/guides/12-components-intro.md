...
组件层之下是 **173 原语 SSOT**（layout / ui / shell / gesture / capability / engineering 六族）：组件是原语的 `p-*` 组件形态，能力 Hook（`useXxx`）是原语的 API 形态——两层共享同一份语义清单。

## 组件分类总表

清单唯一聚合点：`src/components/index.ts`（69 个语义组件：`p-*` 语义组件 + 玻璃容器 `pg-glass`）。
