...
├─ 语义层（框架核心）  p-* 语义组件 / 142 原语 SSOT / Capability Hook / Fluid
...
└─ 能力层             NativeBackend SPI（📋 规划）+ Capability Hook（51 个）
```
...
| 组件框架 | 组件化与审计 / 引用注册 / 生命周期与事件 / 样式与插槽 |
| 运行期 | Web 运行时 / 小程序运行时 / 启动流程 |
| 渲染层 | RenderBackend SPI + Flutter / Headless 后端 |
| 宿主与内存 | 容器与宿主、所有权工程 |
| 数据与状态 | 状态工厂四端 / 数据更新策略 / 页面间传递 / 跨端协同 |
| 质量与兼容 | 一致性验证、体积预算、语义版本 |
| 基础能力 | 网络、存储、分包与按需注入 |
