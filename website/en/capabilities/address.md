...
useAddress: shipping address picker (wx.chooseAddress; web has no standard → Err)

> Capability primitive C54 · `capability.address` · returns `ShippingAddress` · **Hook implemented** (API ready — target bridges in the table below)
...
| `data` | `ShippingAddress` | Success payload (structure below) |
...
| `provinceName` | `string` | Yes | — |
| `cityName` | `string` | Yes | — |
| `countyName` | `string` | Yes | — |
| `detailInfo` | `string` | Yes | — |
...
| code | Doc |
|---|---|
| `address.unsupported` | The bridge does not provide chooseAddress (useAddress unavailable) |
| `address.failed` | wx.chooseAddress failed |
...
| Target | Status | Notes |
|---|---|---|
| Web SPA | ⚠️ | vue-dom · webBridge missing chooseAddress → explicit Err degradation (no direct platform API) |
| WeChat Mini Program | ✅ | skyline (WebView fallback) · wx bridge → wx.chooseAddress |
