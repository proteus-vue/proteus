...
useSMS: Send SMS (wx restricted — no open API / web has no standard → Err)

> Capability primitive C22 · `capability.sms` · returns `Result<void>` · **Hook implemented** (API ready — target bridges in the table below)
...
| code | Doc |
|---|---|
| `sms.unsupported` | Bridge does not provide sendSMS (useSMS unavailable) |
...
| Target | Status | Notes |
|---|---|---|
| Web SPA | ⚠️ | vue-dom · webBridge missing sendSMS → explicit Err degradation (no direct platform API) |
| WeChat Mini Program | ⚠️ | skyline (WebView fallback) · wx bridge missing sendSMS → explicit Err degradation |
