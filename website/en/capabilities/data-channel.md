...
useDataChannel: data channel (live/real-time — bridged by the host; honest Err fallback by default)

> Capability primitive C31 · `capability.data-channel` · returns `Channel` · **Hook implemented** (API ready — target bridges in the table below)
...
|---|---|---|---|
| `channelId` | `string` | Yes | Channel identifier (business-defined; used for cross-end routing) |
...
| code | Doc |
|---|---|
| `data-channel.unsupported` | Bridge does not provide openDataChannel (useDataChannel unavailable) |
...
| Target | Status | Notes |
|---|---|---|
| Web SPA | ⚠️ | vue-dom · webBridge missing openDataChannel → explicit Err degradation (no direct platform API) |
| WeChat Mini Program | ⚠️ | skyline (WebView fallback) · wx bridge missing openDataChannel → explicit Err degradation |
