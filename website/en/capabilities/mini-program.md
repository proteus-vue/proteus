...
useMiniProgram: jump to a mini program (wx.navigateToMiniProgram / web → Err)

> Capability primitive C47 · `capability.mini-program` · returns `MPContext` · **Hook implemented** (API ready — target bridges in the table below)
...
| code | Doc |
|---|---|
| `mini-program.unsupported` | The bridge does not provide navigateMiniProgram (useMiniProgram unavailable) |
...
| Target | Status | Notes |
|---|---|---|
| Web SPA | ✅ | vue-dom · webBridge platform bridge (injected by default when wx is absent) |
| WeChat Mini Program | ✅ | skyline (WebView fallback) · wx bridge → wx.navigateToMiniProgram |
