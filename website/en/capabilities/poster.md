---
title: usePoster (capability.poster)
group: 通知与分享
order: 6010
---
...
usePoster: share poster — poster generation event (wx.onGeneratePoster; web has no equivalent → empty subscription)

> Capability primitive C80 · `capability.poster` · returns `PosterAPI` · **Hook implemented** (API ready — target bridges in the table below)
...
| `data` | `PosterAPI` | Success payload (methods below) |
...
|---|---|---|---|
| `cb` | `(res: { src: string; promise?: unknown }) => void` | No | 事件回调（src 海报地址 / promise 异步产物） |
...
| code | Doc |
|---|---|
| `poster.unsupported` | The bridge does not provide getPoster (usePoster unavailable) |
...
| WeChat Mini Program | ✅ | skyline (WebView fallback) · wx bridge → wx.onGeneratePoster |
