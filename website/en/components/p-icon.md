...
|---|---|---|
| ui.icon | Content & Forms | `<icon>` (L1 primitive) |
...
| WeChat Mini Program | ✅ | skyline (WebView fallback) · native control mapping → `<icon>` (L1 primitive) |
...
|---|---|---|---|---|
| `name` | Icon name (built-in glyph table; unknown → '?') | `String` | `'info'` | No |
| `size` | Size (px) | `Number` | `16` | No |
...
<!-- generated by website/scripts/gen-content.mjs (en overlay) · source SSOT: src/components/p-icon/index.vue -->