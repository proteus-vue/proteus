...
Foreground/background lifecycle observation (visibilitychange/focus — app phase)
...
## Core exports (SSOT: `packages/desktop/src/lifecycle.ts`)

| Export | Kind | One-liner (source comment) |
|---|---|---|
| `AppPhase` | type | · createLifecycleTracker: Web visibilitychange/focus subscription → onPhase (env-injectable & unit-testable; default env talks directly to document — typeof guard) |
| `LifecycleEnv` | interface | — |
| `phaseOf` | function | Normalizes the foreground/background phase (hidden → background; otherwise foreground) |
| `LifecycleTracker` | interface | — |
| `createLifecycleTracker` | function | ★createLifecycleTracker: subscribes to visibilitychange/focus → onPhase (instant notification when the page switches to background or returns to foreground) |
...
```
> Origin: `examples/pages/semantic-primitives-demo.vue:477`
