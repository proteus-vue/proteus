...
Hover-state semantics (brighten/lift/underline) — touch auto-degrades to tap highlight
...
## Core exports (SSOT: `packages/desktop/src/hover.ts`)

| Export | Kind | One-liner (source comment) |
|---|---|---|
| `HoverPreset` | type | Pure logic with zero DOM dependency; MP artifact safe: no ?. / ??; no array destructuring |
| `PointerKind` | type | — |
| `resolveHoverClass` | function | Preset → CSS class name (the render layer's `<style>` defines the transition animation) |
| `isHoverPointer` | function | Pointer classification: mouse/pen support hover (the cursor can hover); touch/remote degrade (plan: iOS compile-time stripping → tap highlight) |
| `normalizePointerType` | function | Event pointerType normalization (Web PointerEvent.pointerType / test injection) |
| `canHover` | function | Hover-state toggle decision (mouseenter/mouseleave semantics — whether the injected pointer can hover) |
...
```
> Origin: `website/src/pages/Home.vue:244`
