...
Desktop tab-close migration: activation prefers the right neighbor; last tab falls back to its left neighbor
...
## Core exports (SSOT: `packages/desktop/src/tabs.ts`)

| Export | Kind | One-liner (source comment) |
|---|---|---|
| `DesktopTab` | interface | Mapping: UISegmentedControl / TabLayout (01 §7); distinct from mobile p-tabbar (S3 bottom bar)—desktop window tabs (closable; multi-row/scroll handled by the host) |
| `TabCloseResult` | interface | — |
| `resolveTabAfterClose` | function | ★resolveTabAfterClose: active-tab transition after a tab is closed |
| `normalizeTabs` | function | ★normalizeTabs: active-id validity normalization (missing/unknown active id → first tab; empty → null) |
...
```
> Origin: `examples/pages/semantic-primitives-demo.vue:436`
